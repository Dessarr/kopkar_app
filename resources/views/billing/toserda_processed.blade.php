...
@section('title', 'Billing Toserda Diproses')
@section('sub-title', 'Daftar Pembayaran Billing Toserda')
...
<div class="container mx-auto px-4">
    ...
    <div class="bg-white rounded-lg shadow-md mb-6">
        <div class="border-b border-gray-200 px-6 py-4">
            <h5 class="font-semibold text-lg">Riwayat Pembayaran Toserda</h5>
        </div>
        <div class="p-6">
            <h4 class="text-xl font-semibold mb-6">Daftar Billing Toserda Lunas</h4>
            ...
            <div class="mb-6">
                <form action="{{ route('billing.toserda.processed') }}" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4">
            ...
            <div class="overflow-x-auto">
                <table class="w-full">
                    ...
                    <tbody class="divide-y">
                        @forelse($dataBillingProcessed as $index => $billing)
                        <tr class="{{ $index % 2 == 0 ? 'bg-white' : 'bg-gray-50' }}">
                            ...
                            <td class="px-4 py-3 text-center text-sm font-medium">
                                <form action="{{ route('billing.toserda.cancel', $billing->id) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                        ...
                        @empty
                        <tr>
                            <td colspan="7" class="px-4 py-3 text-center text-sm text-gray-500">Tidak ada data billing yang sudah diproses
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
            ...
            <div class="mt-6 flex justify-center">
                @if(method_exists($dataBillingProcessed, 'hasPages') && $dataBillingProcessed->hasPages())
                <div class="pagination-links">
                    {{ $dataBillingProcessed->withQueryString()->links() }}
                </div>
                @endif
            </div>
        </div>
