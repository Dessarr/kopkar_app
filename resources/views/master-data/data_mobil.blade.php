...
<div class="px-1 justify-center flex flex-col">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="p-4 border-b">
            <h2 class="text-lg font-semibold">Riwayat Mobil</h2>
        </div>
            ...
            <table class="table-auto w-full border border-gray-300 text-center">
                <thead class="bg-gray-100">
                    <tr class="text-sm">
                        ...
                        <th class="py-2 border">Nama Mobil</th>
                        <th class="py-2 border">Jenis</th>
                        <th class="py-2 border">Merek</th>
                        <th class="py-2 border">No Polisi</th>
                        <th class="py-2 border">Tgl Berlaku STNK</th>
                ...
                <tbody>
                    ...
                    <tr class="text-sm align-middle">
                        ...
                        <td class="py-2 border">{{ $mobil->nama }}</td>
                        <td class="py-2 border">{{ $mobil->jenis }}</td>
                        <td class="py-2 border">{{ $mobil->merek }}</td>
                        <td class="py-2 border">{{ $mobil->no_polisi }}</td>
                        <td class="py-2 border">{{ $mobil->tgl_berlaku_stnk }}</td>
                        <td class="py-2 border">{{ $mobil->aktif ? 'Aktif' : 'Nonaktif' }}</td>
                    </tr>
                    @endforeach
    ...
    <div class="mt-5 w-full relative px-2 py-2">
        <div class="mx-auto w-fit">
            <div
                ...
                @for ($i = 1; $i <= $dataMobil->lastPage(); $i++)
                    @if ($i == 1 || $i == $dataMobil->lastPage() || ($i >= $dataMobil->currentPage() - 1 && $i <=
                        $dataMobil->
                        currentPage() + 1))
                        <a href="{{ $dataMobil->url($i) }}">
                            <div
                                class="rounded-md px-2 py-0.5 text-sm border border-gray-300 {{ $dataMobil->currentPage() == $i ? 'bg-gray-100 font-bold' : '' }}">
        ...
        <div class="absolute right-4 top-1/2 -translate-y-1/2 whitespace-nowrap text-sm text-gray-400">
                Displaying {{ $dataMobil->firstItem() }} to {{ $dataMobil->lastItem() }} of {{ $dataMobil->total() }} items
        </div>
