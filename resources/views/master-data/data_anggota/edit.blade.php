...
<div class="px-1 justify-center flex flex-col">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Formulir untuk mengedit data anggota koperasi</p>
        </div>
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <form action="{{ route('master-data.data_anggota.update', $anggota->id) }}" method="POST" enctype="multipart/form-data" class="p-6">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <select name="status" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('status') border-red-500 @enderror">
                            ...
                            <option value="Belum Kawin" {{ old('status', $anggota->status) == 'Belum Kawin' ? 'selected' : '' }}>Belum Kawin</option>
                            <option value="Kawin" {{ old('status', $anggota->status) == 'Kawin' ? 'selected' : '' }}>Kawin</option>
                            <option value="Cerai Hidup" {{ old('status', $anggota->status) == 'Cerai Hidup' ? 'selected' : '' }}>Cerai Hidup</option>
                            <option value="Cerai Mati" {{ old('status', $anggota->status) == 'Cerai Mati' ? 'selected' : '' }}>Cerai Mati</option>
                        </select>
                        @error('status')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" name="departement" value="{{ old('departement', $anggota->departement) }}" required
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Status Aktif <span class="text-red-500">*</span></label>
                        <select name="aktif" required class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('aktif') border-red-500 @enderror">
                            <option value="1" {{ old('aktif', $anggota->aktif == 'Y' ? '1' : '0') == '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('aktif', $anggota->aktif == 'Y' ? '1' : '0') == '0' ? 'selected' : '' }}>Tidak Aktif</option>
                        </select>
                        @error('aktif')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="md:col-span-2">
                        ...
                        <textarea name="alamat" rows="3" required
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 @error('alamat') border-red-500 @enderror">{{ old('alamat', $anggota->alamat) }}</textarea>
                        @error('alamat')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama_pemilik_rekening" value="{{ old('nama_pemilik_rekening', $anggota->nama_pemilik_rekening) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="no_rekening" value="{{ old('no_rekening', $anggota->no_rekening) }}" required
            ...
            <div class="mb-8">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div>
                        ...
                        <input type="text" name="simpanan_wajib" value="{{ old('simpanan_wajib', number_format($anggota->simpanan_wajib, 0, ',', '.')) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="simpanan_sukarela" value="{{ old('simpanan_sukarela', number_format($anggota->simpanan_sukarela, 0, ',', '.')) }}" required
                    ...
                    <div>
                        ...
                        <input type="text" name="simpanan_khusus_2" value="{{ old('simpanan_khusus_2', number_format($anggota->simpanan_khusus_2, 0, ',', '.')) }}" required
