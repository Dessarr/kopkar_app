...
@section('sub-title', 'Jenis Angsuran')
...
<div class="px-1 justify-center flex flex-col">
            ...
            <table class="table-auto w-full border border-gray-300 text-center">
                <thead class="bg-gray-100">
                    <tr class="text-sm">
                        ...
                        <th class="py-2 border">Keterangan</th>
                        <th class="py-2 border">Aktif</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($jnsAngsuran as $angsuran)
                    <tr class="text-sm align-middle">
                        <td class="py-2 border">
                            {{ ($jnsAngsuran->currentPage() - 1) * $jnsAngsuran->perPage() + $loop->iteration }}
                        </td>
                        <td class="py-2 border">{{ $angsuran->ket }}</td>
                        <td class="py-2 border">{{ $angsuran->aktif ? 'Aktif' : 'Nonaktif' }}</td>
                    </tr>
                    @endforeach
    ...
    <div class="mt-5 w-full relative px-2 py-2">
        <div class="mx-auto w-fit">
            <div
                ...
                @for ($i = 1; $i <= $jnsAngsuran->lastPage(); $i++)
                    @if ($i == 1 || $i == $jnsAngsuran->lastPage() || ($i >= $jnsAngsuran->currentPage() - 1 && $i <=
                        $jnsAngsuran->currentPage() + 1))
                        <a href="{{ $jnsAngsuran->url($i) }}">
                            <div
                                class="rounded-md px-2 py-0.5 text-sm border border-gray-300 {{ $jnsAngsuran->currentPage() == $i ? 'bg-gray-100 font-bold' : '' }}">
        ...
        <div class="absolute right-4 top-1/2 -translate-y-1/2 whitespace-nowrap text-sm text-gray-400">
            Displaying {{ $jnsAngsuran->firstItem() }} to {{ $jnsAngsuran->lastItem() }} of {{ $jnsAngsuran->total() }} items
        </div>
