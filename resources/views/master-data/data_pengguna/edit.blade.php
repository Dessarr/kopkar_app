...
<div class="px-1 justify-center flex flex-col">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <form action="{{ route('master-data.data_pengguna.update', $pengguna->id) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="md:col-span-2">
                    ...
                    <input type="text" id="u_name" name="u_name" value="{{ old('u_name', $pengguna->u_name) }}" 
                ...
                <div class="md:col-span-2">
                    <label for="pass_word" class="block text-sm font-medium text-gray-700 mb-2">
                        ...
                        <span class="text-gray-500 text-xs">(Kosongkan jika tidak ingin mengubah password)</span>
                    </label>
                ...
                <div>
                    ...
                    <select id="level" name="level" 
                        ...
                        <option value="admin" {{ old('level', $pengguna->level) == 'admin' ? 'selected' : '' }}>Administrator</option>
                        <option value="pinjaman" {{ old('level', $pengguna->level) == 'pinjaman' ? 'selected' : '' }}>Staff Pinjaman</option>
                        <option value="simpanan" {{ old('level', $pengguna->level) == 'simpanan' ? 'selected' : '' }}>Staff Simpanan</option>
                        <option value="kas" {{ old('level', $pengguna->level) == 'kas' ? 'selected' : '' }}>Staff Kas</option>
                        <option value="laporan" {{ old('level', $pengguna->level) == 'laporan' ? 'selected' : '' }}>Staff Laporan</option>
                        <option value="supervisor" {{ old('level', $pengguna->level) == 'supervisor' ? 'selected' : '' }}>Supervisor</option>
                        <option value="manager" {{ old('level', $pengguna->level) == 'manager' ? 'selected' : '' }}>Manager</option>
                    </select>
                    @error('level')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                ...
                <div>
                    ...
                    <select id="id_cabang" name="id_cabang" 
                        ...
                        @foreach($cabangs as $cabang)
                            <option value="{{ $cabang->id_cabang }}" {{ old('id_cabang', $pengguna->id_cabang) == $cabang->id_cabang ? 'selected' : '' }}>
                                {{ $cabang->nama }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="md:col-span-2">
                    ...
                    <div class="flex space-x-4">
                        <label class="flex items-center">
                            <input type="radio" name="aktif" value="Y" {{ old('aktif', $pengguna->aktif) == 'Y' ? 'checked' : '' }} 
                                   class="h-4 w-4 text-[#14AE5C] focus:ring-[#14AE5C] border-gray-300">
                            <span class="ml-2 text-sm text-gray-700">Aktif</span>
                        </label>
                        <label class="flex items-center">
                            <input type="radio" name="aktif" value="N" {{ old('aktif', $pengguna->aktif) == 'N' ? 'checked' : '' }} 
                                   class="h-4 w-4 text-[#14AE5C] focus:ring-[#14AE5C] border-gray-300">
