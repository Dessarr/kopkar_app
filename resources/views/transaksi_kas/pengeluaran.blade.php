...
<div class="px-1 justify-center flex flex-col">
    ...
    <div class="bg-white rounded-lg shadow-md p-3 mb-4">
        <div class="flex items-center justify-between mb-2">
            <h3 class="text-base font-semibold text-gray-800">Filter Pengeluaran Kas</h3>
        </div>
            ...
            <table class="table-auto w-full border border-gray-300 text-center">
                <thead class="bg-gray-100">
                    <tr class="text-sm">
                        ...
                        <th class="py-3 border px-4">Untuk Akun</th>
                ...
                <tbody>
                    ...
                    <tr class="text-sm align-middle hover:bg-gray-50 cursor-pointer row-selectable"
                        data-id="{{ $kas->id }}" data-kode="TKK{{ str_pad($kas->id, 5, '0', STR_PAD_LEFT) }}"
                        data-tanggal="{{ $kas->tgl_catat }}" data-keterangan="{{ $kas->keterangan }}"
                        data-kas-id="{{ $kas->dari_kas_id }}" data-kas-nama="{{ $kas->dariKas->nama ?? '-' }}"
                        ...
                        <td class="py-3 border px-4">{{ \Carbon\Carbon::parse($kas->tgl_catat)->format('d/m/Y H:i') }}
                        </td>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        ...
                        <select name="akun" required class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                            ...
                            <option value="{{ $akun->id }}">{{ $akun->jns_trans }}</option>
                            @endforeach
                        </select>
                    </div>
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div class="flex items-center justify-between p-4 border-b">
                <h3 class="text-lg font-semibold">Edit Pengeluaran Kas</h3>
...
function doSearch() {
    ...
    if (kodeTransaksi && kodeTransaksi.trim() !== '') {
        let cleanCode = kodeTransaksi.replace(/TKK/gi, '').replace(/^0+/, '');
    ...
    window.location.href = "{{ route('admin.transaksi.pengeluaran') }}?" + params.toString();
}
...
function cetakLaporan() {
    ...
    window.open("{{ route('admin.transaksi.pengeluaran.export.pdf') }}?" + params.toString(), '_blank');
}
...
function editData() {
    ...
    document.getElementById('edit_akun').value = selectedRowData.akun_id || '';
}
...
function deleteData() {
    ...
    if (confirm(`Apakah Anda yakin ingin menghapus data kode transaksi: ${selectedRowData.kode}?`)) {
        ...
        const deleteUrl = `{{ url('transaksi-kas/pengeluaran') }}/${selectedRowData.id}`;
        fetch(deleteUrl, {
                method: 'DELETE',
                headers: {
...
document.getElementById('addForm').addEventListener('submit', function(e) {
    ...
    if (!data.akun || data.akun === '') {
        alert('Jenis Akun harus dipilih');
        return;
    }
...
document.getElementById('editForm').addEventListener('submit', function(e) {
    ...
    const updateUrl = `{{ url('transaksi-kas/pengeluaran') }}/${selectedRowData.id}`;
    fetch(updateUrl, {
            method: 'PUT',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
