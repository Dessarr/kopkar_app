...
<div class="px-1 justify-center flex flex-col">
    ...
    <div class="bg-white rounded-lg shadow-md p-3 mb-4">
        <div class="flex items-center justify-between mb-2">
            <h3 class="text-base font-semibold text-gray-800">Filter Pembelian Toserda</h3>
        </div>
                ...
                <tbody>
                    ...
                    <tr>
                        <td colspan="8" class="py-8 text-center text-gray-500">
                            ...
                            <p>Tidak ada data pembelian toserda</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div class="flex items-center justify-between p-4 border-b">
                <h3 class="text-lg font-semibold">Tambah Pembelian Toserda</h3>
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div class="flex items-center justify-between p-4 border-b">
                <h3 class="text-lg font-semibold">Edit Pembelian Toserda</h3>
...
function doSearch() {
    ...
    window.location.href = "{{ route('toserda.pembelian') }}?" + params.toString();
}
...
function cetakLaporan() {
    ...
    // Redirect ke route export pembelian
    window.open("{{ route('toserda.pembelian.export.pdf') }}?" + params.toString(), '_blank');
}
...
function deleteData() {
    ...
    if (confirm(`Apakah Anda yakin ingin menghapus data kode transaksi: ${selectedRowData.kode}?`)) {
        ...
        const deleteUrl = `{{ url('toserda/pembelian') }}/${selectedRowData.id}`;
        fetch(deleteUrl, {
                method: 'DELETE',
                headers: {
...
document.getElementById('addForm').addEventListener('submit', function(e) {
    ...
    // Submit data
    fetch("{{ route('toserda.store.pembelian') }}", {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                'Content-Type': 'application/json',
...
document.getElementById('editForm').addEventListener('submit', function(e) {
    ...
    const updateUrl = `{{ url('toserda/pembelian') }}/${selectedRowData.id}`;
    fetch(updateUrl, {
            method: 'PUT',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
