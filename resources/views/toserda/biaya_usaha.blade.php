...
<div class="px-1 justify-center flex flex-col">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    <p class="text-sm font-medium text-gray-600">Total Biaya Usaha</p>
                    <p class="text-2xl font-semibold text-gray-900">
                        Rp{{ number_format($totalBiayaUsaha, 0, ',', '.') }}
                    </p>
                </div>
            </div>
        </div>
    ...
    <div class="bg-white rounded-lg shadow-md p-3 mb-4">
        <div class="flex items-center justify-between mb-2">
            <h3 class="text-base font-semibold text-gray-800">Filter Biaya Usaha Toserda</h3>
        </div>

        <form method="GET" action="{{ route('toserda.biaya-usaha') }}" id="filterForm">
            <!-- Simple Filter Bar -->
                ...
                <div class="flex items-center space-x-3">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <button type="button" id="daterange-btn"
                            class="px-3 py-1.5 border border-gray-300 rounded-lg bg-white hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm">
                    ...
                    <div class="flex items-center space-x-2">
                        ...
                        <input type="text" name="kode_transaksi" id="kode_transaksi"
                            ...
                            class="px-2 py-1.5 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm w-36"
                            onkeypress="if(event.key==='Enter'){doSearch();}">
                    </div>

                    <!-- 3. Button Filter -->
                    <button type="button" onclick="doSearch()" id="searchBtn"
                        class="px-3 py-1.5 bg-orange-600 text-white rounded-lg hover:bg-orange-700 transition-colors text-sm">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="flex justify-between items-center mb-2 p-4">
            <h2 class="text-lg font-semibold text-gray-800">Data Biaya Usaha Toserda</h2>
            ...
            <table class="table-auto w-full border border-gray-300 text-center">
                ...
                <tbody>
                    ...
                    <tr class="text-sm align-middle hover:bg-gray-50 cursor-pointer row-selectable"
                        ...
                        data-kas-id="{{ $tr->dari_kas_id }}" data-kas-nama="{{ optional($tr->dariKas)->nama ?? '-' }}"
                        ...
                        <td class="py-3 border px-4 font-semibold text-orange-600">
                    ...
                    <tr>
                        <td colspan="8" class="py-8 text-center text-gray-500">
                            ...
                            <p>Tidak ada data biaya usaha toserda</p>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div class="flex items-center justify-between p-4 border-b">
                <h3 class="text-lg font-semibold">Tambah Biaya Usaha Toserda</h3>
            ...
            <form id="addForm" class="p-4">
                <div class="space-y-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">Kas Sumber</label>
                    ...
                    <div>
                        ...
                        <select name="jns_trans" required class="w-full px-3 py-2 border border-gray-300 rounded-lg">
                            ...
                            <option value="122">Beban Gaji Karyawan</option>
                            <option value="123">Biaya Operasional</option>
                            <option value="124">Bahan Habis Pakai</option>
                            <option value="125">Insentive Karyawan</option>
                        </select>
                    </div>
                </div>
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-md">
            <div class="flex items-center justify-between p-4 border-b">
                <h3 class="text-lg font-semibold">Edit Biaya Usaha Toserda</h3>
...
function doSearch() {
    ...
    window.location.href = "{{ route('toserda.biaya-usaha') }}?" + params.toString();
}
...
function clearFilters() {
    ...
    // Reload page
    window.location.href = "{{ route('toserda.biaya-usaha') }}";
}
...
function cetakLaporan() {
    ...
    // Redirect ke route export biaya usaha
    window.open("{{ route('toserda.biaya-usaha.export.pdf') }}?" + params.toString(), '_blank');
}
...
function editData() {
    ...
    document.getElementById('edit_dari_kas_id').value = selectedRowData.kas_id;
...
function deleteData() {
    ...
    if (confirm(`Apakah Anda yakin ingin menghapus data kode transaksi: ${selectedRowData.kode}?`)) {
        ...
        const deleteUrl = `{{ url('toserda/biaya-usaha') }}/${selectedRowData.id}`;
        fetch(deleteUrl, {
                method: 'DELETE',
                headers: {
...
document.getElementById('addForm').addEventListener('submit', function(e) {
    ...
    if (!data.dari_kas_id || data.dari_kas_id === '') {
        alert('Kas Sumber harus dipilih');
        return;
    }
    ...
    // Submit data
    fetch("{{ route('toserda.store.biaya-usaha') }}", {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
                'Content-Type': 'application/json',
...
document.getElementById('editForm').addEventListener('submit', function(e) {
    ...
    const updateUrl = `{{ url('toserda/biaya-usaha') }}/${selectedRowData.id}`;
    fetch(updateUrl, {
            method: 'PUT',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content'),
