            ...
            <table class="w-full table-fixed border border-gray-200 text-[12px]">
                <thead class="bg-gray-50 text-[12px] uppercase text-gray-600">
                    <tr class="w-full">
                        ...
                        <th class="py-2 px-3 border text-center w-[80px]">Bunga %</th>
                ...
                <tbody class="divide-y divide-gray-100">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="py-1 px-2 border align-top">
                            {{ $pinjaman->jenis_pinjaman == '1' ? 'Biasa' : 'Barang' }}
                        </td>
                        ...
                        <td class="py-1 px-2 border text-center align-top">{{ $pinjaman->bunga }}%</td>
                        ...
                        <td class="py-1 px-2 border align-top">
                            <div class="grid grid-cols-2 gap-1">
                                <a class="w-full px-1.5 py-0.5 text-[10px] rounded border bg-blue-50 text-blue-700 border-blue-300 hover:bg-blue-100"
                                    href="{{ route('pinjaman.data_pinjaman.show', $pinjaman->id) }}">Detail</a>
                                <a class="w-full px-1.5 py-0.5 text-[10px] rounded border bg-green-50 text-green-700 border-green-300 hover:bg-green-100"
                                    href="{{ route('pinjaman.data_angsuran.show', $pinjaman->id) }}">Angsuran</a>
                                <a class="w-full px-1.5 py-0.5 text-[10px] rounded border bg-yellow-50 text-yellow-700 border-yellow-300 hover:bg-yellow-100"
                                    href="{{ route('pinjaman.data_pinjaman.edit', $pinjaman->id) }}">Edit</a>
                                <form action="{{ route('pinjaman.data_pinjaman.destroy', $pinjaman->id) }}"
                                    method="POST" onsubmit="return confirm('Hapus pinjaman ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button
                                        class="w-full px-1.5 py-0.5 text-[10px] rounded border bg-red-50 text-red-700 border-red-300 hover:bg-red-100">Hapus</button>
                                </form>
                            </div>
                        </td>
                    </tr>
