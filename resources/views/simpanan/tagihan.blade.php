...
@section('title', 'Transaksi Tagihan Setoran')
@section('sub-title', 'Transaksi Tagihan Setoran')
...
<div class="px-1 justify-center flex flex-col">
            ...
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">
                        Rp{{ number_format($tagihan->sum('jumlah'), 0, ',', '.') }}
                    </p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-lg shadow p-6">
            <div class="flex items-center">
                ...
                <div class="ml-4">
                    ...
                    <p class="text-2xl font-semibold text-gray-900">{{ $tagihan->total() }}</p>
                </div>
            </div>
        </div>
        <div class="bg-white rounded-lg shadow p-6">
    ...
    <div class="bg-white rounded-lg shadow-md p-3 mb-4">
        <div class="flex items-center justify-between mb-2">
            <h3 class="text-base font-semibold text-gray-800">Filter Tagihan Setoran</h3>
        </div>

        <form method="GET" action="{{ route('simpanan.tagihan') }}" id="filterForm">
    ...
    <div class="bg-white rounded-lg shadow overflow-hidden">
        <div class="flex justify-between items-center mb-2 p-4">
            <h2 class="text-lg font-semibold text-gray-800">Data Transaksi Tagihan Setoran</h2>
            ...
            <table class="table-auto w-full border border-gray-300 text-center">
                ...
                <tbody>
                    ...
                    <tr class="text-sm align-middle hover:bg-gray-50 cursor-pointer row-selectable"
                        ...
                        data-kode="TRD{{ str_pad($t->id, 5, '0', STR_PAD_LEFT) }}"
                        ...
                        <td class="py-3 border px-4">
                            {{ $t->tgl_transaksi ? \Carbon\Carbon::parse($t->tgl_transaksi)->format('d F Y') : '-' }}
                        </td>
                        <td class="py-3 border px-4">
    ...
    <div class="mt-5 w-full relative px-2 py-2">
        <div class="mx-auto w-fit">
            <div
                ...
                @for ($i = 1; $i <= $tagihan->lastPage(); $i++)
                    @if ($i == 1 || $i == $tagihan->lastPage() || ($i >= $tagihan->currentPage() - 1 && $i <= $tagihan->
                        currentPage() + 1))
                        <a href="{{ $tagihan->appends(request()->query())->url($i) }}">
                            <div
                                class="rounded-md px-2 py-0.5 text-sm border border-gray-300 {{ $tagihan->currentPage() == $i ? 'bg-gray-100 font-bold' : '' }}">
                                {{ str_pad($i, 2, '0', STR_PAD_LEFT) }}
                            </div>
                        </a>
                        @elseif ($i == 2 || $i == $tagihan->lastPage() - 1)
        ...
        <div class="absolute right-4 top-1/2 -translate-y-1/2 whitespace-nowrap text-sm text-gray-400">
            Displaying {{ $tagihan->firstItem() }} to {{ $tagihan->lastItem() }} of {{ $tagihan->total() }} items
        </div>
    </div>
</div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-4 border-b">
                <h3 class="text-lg font-semibold">Tambah Data Tagihan</h3>
            ...
            <form id="addForm" class="p-4">
                ...
                <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    <!-- Data Tagihan -->
                    <div class="space-y-4">
                        <h4 class="text-md font-semibold text-gray-800 border-b pb-2">Data Tagihan</h4>
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden z-50">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg shadow-xl w-full max-w-4xl max-h-[90vh] overflow-y-auto">
            <div class="flex items-center justify-between p-4 border-b">
                <h3 class="text-lg font-semibold">Edit Data Tagihan</h3>
...
function editData() {
    ...
    document.getElementById('edit_tagihan_id').value = selectedRowData.id;
...
function deleteData() {
    ...
    if (confirm(`Apakah Anda yakin ingin menghapus data kode transaksi: ${selectedRowData.kode}?`)) {
        const deleteUrl = `{{ url('simpanan/tagihan') }}/${selectedRowData.id}/delete`;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (addForm) {
        addForm.addEventListener('submit', async function(e) {
            ...
            try {
                const response = await fetch("{{ route('simpanan.tagihan.store') }}", {
                    method: 'POST',
                    headers: {
    ...
    if (editForm) {
        editForm.addEventListener('submit', async function(e) {
            ...
            try {
                const updateUrl = `{{ url('simpanan/tagihan') }}/${selectedRowData.id}`;
...
function doSearch() {
    ...
    if (search && search.trim() !== '') {
        let cleanSearch = search.replace(/TRD|AG/gi, '').replace(/^0+/, '');
    ...
    window.location.href = "{{ route('simpanan.tagihan') }}?" + params.toString();
}
...
function clearFilters() {
    ...
    // Reload page
    window.location.href = "{{ route('simpanan.tagihan') }}";
}
...
function cetakLaporan() {
    ...
    window.open("{{ route('simpanan.tagihan.export') }}?" + params.toString(), '_blank');
}
