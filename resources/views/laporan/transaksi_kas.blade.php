    ...
    <div id="filterSection" class="mb-6 bg-gray-50 rounded-lg p-4 border border-gray-200">
        <form method="GET" action="{{ route('laporan.transaksi.kas') }}" class="space-y-4">
            ...
            <div class="flex flex-wrap gap-2 pt-4 border-t border-gray-200">
                <button type="submit"
                    class="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition-colors duration-200 flex items-center">
                    <i class="fas fa-search mr-2"></i>Cari
                </button>
                <a href="{{ route('laporan.transaksi.kas') }}"
                    class="px-6 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 transition-colors duration-200 flex items-center">
            ...
            <table class="min-w-full border border-gray-200">
                <thead class="bg-gradient-to-r from-blue-600 to-blue-700 text-white">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider">Tanggal Transaksi
                        </th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="bg-yellow-50 border-b-2 border-yellow-200">
                        ...
                        <td
                            class="px-4 py-3 text-sm font-bold text-right {{ $saldoSebelumnya >= 0 ? 'text-green-600' : 'text-red-600' }}">
                            Rp {{ number_format($saldoSebelumnya, 0, ',', '.') }}
                        </td>
                    </tr>

                    @forelse($processedData as $index => $transaksi)
                    <tr class="hover:bg-blue-50 transition-colors duration-200">
                        ...
                        <td
                            class="px-4 py-3 text-sm text-right {{ $transaksi->debet > 0 ? 'text-green-600 font-semibold' : 'text-gray-400' }}">
                            {{ $transaksi->debet > 0 ? 'Rp ' . number_format($transaksi->debet, 0, ',', '.') : '-' }}
                        </td>
                        <td
                            class="px-4 py-3 text-sm text-right {{ $transaksi->kredit > 0 ? 'text-red-600 font-semibold' : 'text-gray-400' }}">
                            {{ $transaksi->kredit > 0 ? 'Rp ' . number_format($transaksi->kredit, 0, ',', '.') : '-' }}
                        </td>
                        <td
                            class="px-4 py-3 text-sm font-bold text-right {{ $transaksi->saldo >= 0 ? 'text-green-600' : 'text-red-600' }}">
    ...
    <div class="mt-6 p-4 bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg border border-gray-200">
        <div class="flex flex-wrap justify-between items-center text-sm text-gray-600">
            ...
            <div class="flex items-center">
                ...
                <span class="font-medium">Halaman:</span> {{ $dataTransaksi->currentPage() }} dari
                {{ $dataTransaksi->lastPage() }}
            </div>
            <div class="flex items-center">
                <i class="fas fa-list mr-2 text-purple-500"></i>
                <span class="font-medium">Menampilkan:</span> {{ $dataTransaksi->firstItem() ?? 0 }} -
                {{ $dataTransaksi->lastItem() ?? 0 }} dari {{ $dataTransaksi->total() }}
            </div>
        </div>
    </div>
</div>
