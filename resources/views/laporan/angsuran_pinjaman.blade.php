...
<div class="bg-white rounded-lg shadow-md p-6">
                ...
                <form method="GET" action="{{ route('laporan.angsuran_pinjaman') }}" class="space-y-4">
                    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            ...
                            <select onchange="setPreset(this.value)" class="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500 focus:border-transparent">
    ...
    <div class="mb-6 flex flex-wrap gap-3">
        ...
        <a href="{{ route('laporan.angsuran_pinjaman') }}" class="px-4 py-2 bg-gray-500 text-white rounded-md hover:bg-gray-600 transition-colors duration-200 shadow-sm">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-gradient-to-r from-green-500 to-green-600 text-white p-4 rounded-lg shadow-lg">
            <div class="flex items-center">
                <div class="flex-1">
                    <div class="text-sm font-medium text-green-100">Total Angsuran</div>
                    <div class="text-2xl font-bold">{{ number_format($summary['total_angsuran']) }}</div>
                </div>
        ...
        <div class="bg-gradient-to-r from-blue-500 to-blue-600 text-white p-4 rounded-lg shadow-lg">
            <div class="flex items-center">
                <div class="flex-1">
                    <div class="text-sm font-medium text-blue-100">Total Pokok</div>
                    <div class="text-2xl font-bold">Rp {{ number_format($summary['total_pokok'], 0, ',', '.') }}</div>
                </div>
        ...
        <div class="bg-gradient-to-r from-yellow-500 to-yellow-600 text-white p-4 rounded-lg shadow-lg">
            <div class="flex items-center">
                <div class="flex-1">
                    <div class="text-sm font-medium text-yellow-100">Total Bunga</div>
                    <div class="text-2xl font-bold">Rp {{ number_format($summary['total_bunga'], 0, ',', '.') }}</div>
                </div>
        ...
        <div class="bg-gradient-to-r from-purple-500 to-purple-600 text-white p-4 rounded-lg shadow-lg">
            <div class="flex items-center">
                <div class="flex-1">
                    <div class="text-sm font-medium text-purple-100">Total Denda</div>
                    <div class="text-2xl font-bold">Rp {{ number_format($summary['total_denda'], 0, ',', '.') }}</div>
                </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <div class="text-lg font-semibold text-gray-900">{{ $summary['angsuran_lunas'] }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    <div class="text-sm font-medium text-gray-500">Tepat Waktu</div>
                    <div class="text-lg font-semibold text-gray-900">{{ $summary['angsuran_tepat_waktu'] }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <div class="text-lg font-semibold text-gray-900">{{ $summary['angsuran_terlambat'] }}</div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
            <div class="flex items-center">
                ...
                <div class="ml-3">
                    ...
                    <div class="text-lg font-semibold text-gray-900">{{ $summary['angsuran_belum_bayar'] }}</div>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            ...
            <p class="text-sm text-gray-600 mt-1">Detail pembayaran angsuran periode {{ \Carbon\Carbon::parse($tgl_dari)->format('d M Y') }} - {{ \Carbon\Carbon::parse($tgl_samp)->format('d M Y') }}</p>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Pinjaman Awal</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">JW</th>
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">%</th>
                        <th class="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Saldo Pinjaman</th>
                        ...
                        <th class="px-6 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider">Angsuran Ke</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-gray-50 transition-colors duration-150">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">{{ $row['lama_angsuran'] }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">{{ $row['jumlah_bunga'] }}%</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">Rp {{ number_format($row['saldo_pinjaman'], 0, ',', '.') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">Rp {{ number_format($row['pokok'], 0, ',', '.') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">Rp {{ number_format($row['bunga'], 0, ',', '.') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">Rp {{ number_format($row['denda'], 0, ',', '.') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right font-semibold">Rp {{ number_format($row['jumlah_angsuran'], 0, ',', '.') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">Rp {{ number_format($row['saldo_akhir'], 0, ',', '.') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">{{ $row['angsuran_ke'] }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-center">{{ \Carbon\Carbon::parse($row['tgl_bayar'])->format('d/m/Y') }}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-center">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                @if($row['status'] == 'Lunas') bg-green-100 text-green-800
                                @elseif($row['status'] == 'Tepat Waktu') bg-blue-100 text-blue-800
                    ...
                    <tr>
                        <td colspan="16" class="px-6 py-12 text-center text-gray-500">
                            ...
                            <div class="text-lg font-medium">Tidak ada data angsuran</div>
                            <div class="text-sm">Tidak ada data angsuran untuk periode yang dipilih</div>
                        </td>
                    </tr>
                    @endforelse
                </tbody>
                ...
                <tfoot class="bg-gray-50">
                    <tr class="font-semibold">
                        <td class="px-6 py-4 text-center" colspan="8">TOTAL</td>
                        <td class="px-6 py-4 text-right">Rp {{ number_format($summary['total_pokok'], 0, ',', '.') }}</td>
                        <td class="px-6 py-4 text-right">Rp {{ number_format($summary['total_bunga'], 0, ',', '.') }}</td>
                        <td class="px-6 py-4 text-right">Rp {{ number_format($summary['total_denda'], 0, ',', '.') }}</td>
                        <td class="px-6 py-4 text-right">Rp {{ number_format($summary['total_jumlah_angsuran'], 0, ',', '.') }}</td>
    ...
    <div class="mt-8 bg-white rounded-lg shadow-sm border border-gray-200">
        <div class="px-6 py-4 border-b border-gray-200">
            <h3 class="text-lg font-semibold text-gray-900">
                <i class="fas fa-history mr-2 text-green-600"></i>Aktivitas Angsuran Terbaru
            </h3>
            <p class="text-sm text-gray-600 mt-1">10 angsuran terbaru dalam periode ini</p>
        </div>
            ...
            <div class="space-y-3">
                ...
                <div class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors">
                    <div class="flex items-center">
                        ...
                        <div class="ml-3">
                            <div class="text-sm font-medium text-gray-900">{{ $installment['anggota'] }}</div>
                            <div class="text-sm text-gray-500">{{ $installment['id'] }} • {{ $installment['tgl_bayar'] }}</div>
                        </div>
                    </div>
                    <div class="text-right">
                        <div class="text-sm font-medium text-gray-900">Rp {{ number_format($installment['jumlah_angsuran'], 0, ',', '.') }}</div>
                        <div class="text-sm text-gray-500">Angsuran ke-{{ $installment['angsuran_ke'] }}</div>
                    </div>
                </div>
                @endforeach
            </div>
