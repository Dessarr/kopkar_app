...
class Lap_laba_m extends CI_Model {
	...
	function get_jml_angsuran() {
		...
		//$this->db->where('DATE(tbl_pinjaman_h.tgl_pinjam) >= ', ''.$tgl_dari.'');
		//$this->db->where('DATE(tbl_pinjaman_h.tgl_pinjam) <= ', ''.$tgl_samp.'');
		$hasil = $this->db->query("SELECT SUM(b.jumlah_bayar) AS jml_total FROM tbl_pinjaman_d b left JOIN tbl_pinjaman_h a on a.id=b.pinjam_id where (DATE(a.tgl_pinjam) >= '$tgl_dari' or DATE(a.tgl_pinjam) <= '$tgl_samp')")->row();
        return $hasil;
	}
