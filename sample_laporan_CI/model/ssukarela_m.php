...
class Ssukarela_m extends CI_Model {
	...
	function get_data_transaksi_ajax($offset, $limit, $q='', $sort, $order) {
		$sql = "SELECT * FROM v_ssukarela where jns_simpan = 'Simpanan Sukarela' ";
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
				...
				if($q['cari_ssukarela'] != '') {
					$sql .=" AND jenis_id = '".$q['cari_ssukarela']."%' ";
				}
	...
	public function view() {
		...
		$view = $this->db->query("SELECT * FROM v_ssukarela where YEAR(tgl_transaksi) = '$thn' and MONTH(tgl_transaksi) = '$bln'");
        return $view;
	}
	...
	public function load() {
		...
		$this->db->query("DELETE FROM `tbl_trans_tagihan_sukarela_temp` WHERE YEAR(tgl_transaksi) = '$thn' and MONTH(tgl_transaksi) = '$bln'");
		
		$load = $this->db->query("INSERT INTO tbl_trans_tagihan_sukarela_temp (tgl_transaksi, no_ktp, anggota_id, jenis_id, jumlah, keterangan, akun, dk) 
			SELECT '$thn-$bln-01', a.no_ktp, a.id, '32', a.simpanan_sukarela, '', 'Tagihan', 'D'
			FROM tbl_anggota a 
			WHERE a.aktif='Y' AND a.simpanan_sukarela <> 0");
        return $load;
	}
	...
	public function ins() {
		...
		$this->db->query("DELETE FROM tbl_trans_tagihan WHERE YEAR(tgl_transaksi) = '$thn' and MONTH(tgl_transaksi) = '$bln' and jenis_id = '32'");
		$this->db->query("INSERT INTO tbl_trans_tagihan (tgl_transaksi, no_ktp, anggota_id, jenis_id, jumlah, keterangan, akun, dk) 
			...
			FROM tbl_trans_tagihan_sukarela_temp 
			WHERE YEAR(tgl_transaksi) = '$thn' and MONTH(tgl_transaksi) = '$bln'");

		$load = $this->db->query("DELETE FROM `tbl_trans_tagihan_sukarela_temp` WHERE YEAR(tgl_transaksi) = '$thn' and MONTH(tgl_transaksi) = '$bln'");
        return $load;
	}
	...
	public function create() {
		...
		return $this->db->insert('tbl_trans_tagihan_sukarela_temp', $data);
	}
	...
	public function update($id) {
		...
		$tanggal_u = date('Y-m-d H:i');
		$this->db->where('id', $id);
		return $this->db->update('tbl_trans_tagihan_sukarela_temp',array(
			'tgl_transaksi'		=>	$this->input->post('tgl_transaksi'),
			'no_ktp'			=>	$this->input->post('no_ktp'),
			'jenis_id'			=>	32,
			'jumlah'			=>	str_replace(',', '', $this->input->post('jumlah')),
	...
	public function delete($id) {
		return $this->db->delete('tbl_trans_tagihan_sukarela_temp', array('id' => $id)); 
	}
}