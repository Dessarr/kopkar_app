...
class Member_m extends CI_Model {
	...
	public function cek_user() {
		...
		$pass_word = sha1($this->input->post('pass_word'));

		$this->db->where('no_ktp', $u_name);
	...
	function cek_pass_lama($user_id) {
		$out = array();
		$pass_word = sha1($this->input->post('password_lama'));
	...
	function simpan() {
		...
		if($data_user){
			$pass_word = sha1($this->input->post('password_baru'));
	...
	public function validasi_penarikan() {
		$form_rules = array(
			array(
				'field' => 'nominal',
				'label' => 'Nominal',
	...
	function pengajuan_simpan() {
		...
		// ajuan_id
		$ajuan_id = '';
		if($pinjaman == 'bunga_biasa') {
			$ajuan_id .= 'B';
			$ajuan .= '1';
		}
		
		if($pinjaman == 'bunga_barang') {
			$ajuan_id .= 'BR';
			$ajuan .= '3';
		}
		if(date("d") >= 21) {
	...
	function pengajuan_penarikan() {
		...
		$this->db->select_max('no_ajuan');
		$this->db->from('tbl_pengajuan_penarikan');
		$this->db->where('DATE(tgl_input) >=', $bln_1);
		$this->db->where('DATE(tgl_input) <=', $bln_2);
		//$this->db->where('jenis', $jenis);
		$query = $this->db->get();
		...
		if($this->db->insert('tbl_pengajuan_penarikan', $data)) {
			// ok
			return TRUE;
		} else {
			return FALSE;
	...
	function pengajuan_batal_penarikan($id) {
		$user_id = $this->session->userdata('u_name');
		$data = array('status' => 4);
		$this->db->where('id', $id);
		$this->db->where('status', 0);
		$this->db->where('anggota_id', $user_id);
		if($this->db->update('tbl_pengajuan_penarikan', $data)) {
			// ok
			return TRUE;
		} else {
			return FALSE;
		...
		if(is_array($data)) {
			...
			foreach ($data as $rows) {
				...
				foreach ($rows as $key => $val) {
					...
					if($key == 'G') { $pair['status'] = $val; }
					if($key == 'H') { $pair['agama'] = $val; }
					if($key == 'I') { $pair['departement'] = $val; }
					if($key == 'J') { $pair['pekerjaan'] = $val; }
					if($key == 'K') { $pair['alamat'] = $val; }
					if($key == 'L') { $pair['kota'] = $val; }
					if($key == 'M') { $pair['notelp'] = $val; }
					if($key == 'N') { $pair['tgl_daftar'] = $val; }
					if($key == 'O') { $pair['jabatan_id'] = $val; }
					if($key == 'P') { $pair['id_tagihan'] = $val; }
					if($key == 'Q') { $pair['pass_word'] = sha1('kao2021'); }
					if($key == 'R') { $pair['aktif'] = $val; }
					//if($key == 'S') { $pair['id_cabang'] = $id_cab; }
				}
				$pair['jabatan_id'] = 2;
				$pair_arr[] = $pair;
			}
	...
	function get_pengajuan_penarikan() {
		$this->load->helper('fungsi');
		$user_id = $this->session->userdata('u_name');
		...
		$sql_tampil = "SELECT * FROM tbl_pengajuan_penarikan WHERE 1=1 ".$where." ".$order_by." ".$sql_limit."";
		$query = $this->db->query($sql_tampil);
		$data_list = $query->result();

		$sql_total = "SELECT id FROM tbl_pengajuan_penarikan WHERE 1=1 ".$where." ";
		$query = $this->db->query($sql_total);
		$total = $query->num_rows();
	...
	function get_pinjaman() {
		...
		$sql_tampil = "SELECT * FROM v_hitung_pinjaman_3 WHERE 1=1 and status='1' ".$where." ".$order_by." ".$sql_limit."";
		$query = $this->db->query($sql_tampil);
		$data_list = $query->result();

		$sql_total = "SELECT id FROM v_hitung_pinjaman_3 WHERE 1=1 and status='1' ".$where." ";
		$query = $this->db->query($sql_total);
		$total = $query->num_rows();
	...
	function get_bayar() {
		...
		$sql_tampil = "SELECT 
				...
				tbl_pinjaman_d.bunga AS bunga,
				tbl_pinjaman_d.denda_rp AS denda_rp,
				tbl_pinjaman_d.ket_bayar AS ket_bayar,
				(tbl_pinjaman_d.jumlah_bayar + tbl_pinjaman_d.bunga) as tot,
		...
		foreach ($data_list as $key => $val) {
			...
			$val->bunga = number_format($val->bunga);
			$val->denda_rp = number_format($val->denda_rp);
			$val->tot = number_format($val->tot);
			$data_list_i[$key] = $val;
		}
	...
	function get_bayar_simpanan() {
		$this->load->helper('fungsi');
		$no_ktp = $this->session->userdata('no_ktp');
		$offset = isset($_POST['offset']) ? $_POST['offset'] : 0;
		$limit = isset($_POST['limit']) ? $_POST['limit'] : 10;
		$search = isset($_POST['search']) ? $_POST['search'] : '';
		$where = " AND tbl_trans_sp.no_ktp = " . $no_ktp;
		$order_by = " ORDER BY tbl_trans_sp.tgl_transaksi DESC";
		$sql_limit = " LIMIT ".$offset.",".$limit." ";
		$sql_tampil = "SELECT 
				tbl_trans_sp.tgl_transaksi AS tgl_transaksi,
				jns_simpan.jns_simpan AS jns_simpan,
				tbl_trans_sp.jumlah AS jumlah
			FROM tbl_trans_sp 
			left JOIN jns_simpan ON tbl_trans_sp.jenis_id = jns_simpan.id
			WHERE 1=1 and tbl_trans_sp.akun = 'Setoran' 
			".$where." ".$order_by." ".$sql_limit."";
		$query = $this->db->query($sql_tampil);
		$data_list = $query->result();

		$sql_total = "SELECT tbl_trans_sp.id 
			FROM tbl_trans_sp 
			left JOIN jns_simpan ON tbl_trans_sp.jenis_id = jns_simpan.id
			WHERE 1=1 and tbl_trans_sp.akun = 'Setoran' ".$where." ";
		$query = $this->db->query($sql_total);
		$total = $query->num_rows();
		...
		foreach ($data_list as $key => $val) {
			$tgl_arr = explode(' ', $val->tgl_transaksi);
			$tgl = $tgl_arr[0];
			$val->tgl_transaksi = jin_date_ina($tgl, 'pendek');
			$val->jumlah = number_format($val->jumlah);
			
			if (empty($val->jns_simpan)){
				$val->jns = 'Toserda';
			} else {
				$val->jns = $val->jns_simpan;
			}
			$data_list_i[$key] = $val;
		}
		$out = array('rows' => $data_list_i, 'total' => $total);
