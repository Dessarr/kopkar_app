...
class Simpanan_m extends CI_Model {
		...
		if(is_array($data)) {
			...
			foreach ($data as $rows) {
				...
				foreach ($rows as $key => $val) {
					...
					if($key == 'F') { $pair['akun'] = 'Setoran'; }
					if($key == 'G') { $pair['dk'] = 'D'; }
					if($key == 'H') { $pair['kas_id'] = 1; }
					if($key == 'I') { $pair['update_data'] = $val; }
					if($key == 'J') { $pair['user_name'] = 'admin'; }
					if($key == 'K') { $pair['nama_penyetor'] = $val; }
					if($key == 'L') { $pair['no_identitas'] = $val; }
	...
	function get_data_transaksi_ajax($offset, $limit, $q='', $sort, $order) {
		//$id_cab = $this->session->userdata('id_cabang');
		$sql = "SELECT * FROM v_trans_sp WHERE dk='D' and jenis_id in (8,31,32,40,41,51,52,53) ";
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
				//$q['kode_transaksi'] = str_replace('TRD', '', $q['kode_transaksi']);
				//$q['kode_transaksi'] = str_replace('AG', '', $q['kode_transaksi']);
