...
class Bunga_m extends CI_Model {
	...
	function update_db_view_hpin($tipe) {
		...
		if($tipe == 'A') {
			$sql = " CREATE VIEW `v_hitung_pinjaman` AS 
				select 
					`tbl_pinjaman_h`.`id` AS `id`,
					`tbl_anggota`.`nama` AS `nama`,
					`tbl_anggota`.`no_ktp` AS `no_ktp`,
					...
					sum(`tbl_pinjaman_d`.`denda_rp`) AS `denda_rp`,
					...
					((`tbl_pinjaman_h`.`jumlah` * `tbl_pinjaman_h`.`bunga`) / 100) AS `pokok_bunga`,
					round(ceiling((((`tbl_pinjaman_h`.`jumlah` / `tbl_pinjaman_h`.`lama_angsuran`) * `tbl_pinjaman_h`.`bunga`) / 100)),-(2)) AS `bunga_pinjaman`,
					round(ceiling(((`tbl_pinjaman_h`.`jumlah` / `tbl_pinjaman_h`.`lama_angsuran`) + ((`tbl_pinjaman_h`.`jumlah` * `tbl_pinjaman_h`.`bunga`) / 100))),0) AS `ags_per_bulan`,
					sum(`tbl_pinjaman_d`.`jumlah_bayar`) AS `total_bayar`,(`tbl_pinjaman_h`.`jumlah` - sum(`tbl_pinjaman_d`.`jumlah_bayar`)) AS `sisa_pokok`,
					(`tbl_pinjaman_h`.`tgl_pinjam` + interval `tbl_pinjaman_h`.`lama_angsuran` month) AS `tempo`,
					round(ceiling(`tbl_pinjaman_h`.`jumlah`),0) AS `tagihan`,`tbl_pinjaman_h`.`keterangan` AS `keterangan`,
					`tbl_pinjaman_h`.`barang_id` AS `barang_id`,ifnull(max(`tbl_pinjaman_d`.`angsuran_ke`),0) AS `bln_sudah_angsur`,
					sum(`tbl_pinjaman_d`.`bunga`) AS `bunga_ags` 
				from ((`tbl_pinjaman_h` left join `tbl_pinjaman_d` 
					on((`tbl_pinjaman_h`.`id` = `tbl_pinjaman_d`.`pinjam_id`))) join `tbl_anggota` 
					on((`tbl_anggota`.`no_ktp` = `tbl_pinjaman_h`.`no_ktp`))) 
