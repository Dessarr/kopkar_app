...
class Potongan extends OperatorController {
	...
	public function index() {
		...
		$this->data['judul_sub'] 		= 'Potongan Gaji Karyawan';
		...
		$this->data['isi'] = $this->load->view('potongan_list_v', $this->data, TRUE);
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Potongan <a href="'.site_url('potongan').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_potongan')) {
		...
		$this->data['isi'] = $this->load->view('potongan_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}

	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('potongan_m','potongan', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->potongan->import_db($data_import)) {
	...
	function ajax_list() {
		...
		$cari_potongan = isset($_POST['cari_potongan']) ? $_POST['cari_potongan'] : '';
		$tgl_dari = isset($_POST['tgl_dari']) ? $_POST['tgl_dari'] : '';
		$tgl_sampai = isset($_POST['tgl_sampai']) ? $_POST['tgl_sampai'] : '';
		$search = array('kode_transaksi' => $kode_transaksi, 
			'cari_potongan' => $cari_potongan,
			'tgl_dari' 		=> $tgl_dari, 
			'tgl_sampai' 	=> $tgl_sampai);
		$offset = ($offset-1)*$limit;
		$data   = $this->potongan_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['no_ktp']	 			= $r->no_ktp;
			//$rows[$i]['anggota_id_txt'] 	= 'AG' . sprintf('%04d', $r->no_ktp);
			...
			$rows[$i]['nota'] 				= '<p></p><p>
			<a href="'.site_url('cetak_potongan').'/cetak/' . $r->id . '"  title="Cetak Bukti Potongan" target="_blank"> <i class="glyphicon glyphicon-print"></i> Nota </a></p>';
