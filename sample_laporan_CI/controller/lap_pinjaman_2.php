...
class Lap_pinjaman extends OperatorController {
	...
	public function index() {
		...
		$this->data['judul_browser'] = 'Laporan Pinjaman';
		...
		$this->data['kas_id'] = $this->lap_pinjaman_m->get_data_kas();
		$this->data['jenis_ags'] = $this->lap_pinjaman_m->get_data_angsuran();
		$this->data['suku_bunga'] = $this->lap_pinjaman_m->get_data_bunga();
		$this->data['biaya'] = $this->lap_pinjaman_m->get_biaya_adm();
		$this->data['barang_id'] = $this->lap_pinjaman_m->get_id_barang();
	...
	function ajax_list() {
		...
		$data   = $this->lap_pinjaman_m->get_data_transaksi_ajax($id_cab,$offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		foreach ($data['data'] as $r) {
			...
			$barang = $this->lap_pinjaman_m->get_data_barang($r->barang_id);   
			...
			$rows[$i]['anggota_id'] 		= 'AG' . sprintf('%04d', $r->anggota_id).' <br />'.$anggota->nama;
			$rows[$i]['anggota_id_txt'] 	= 'AG' . sprintf('%04d', $r->anggota_id).' <br />'.$anggota->nama.' <br />'.$anggota->departement;
	...
	function get_jenis_barang() {
		...
		$jenis_barang = $this->lap_pinjaman_m->get_id_barang();
	...
	public function update($id=null) {
		...
		if($this->lap_pinjaman_m->update($id)) {
	...
	public function delete() {
		...
		if($this->lap_pinjaman_m->delete($id)) {
