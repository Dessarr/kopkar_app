...
class Cetak_pinjaman_detail extends OperatorController {
	...
	function cetak($id) {
		...
		<table width="100%" cellspacing="0" cellpadding="3" border="1" border-collapse= "collapse">';
			...
			$anggota = $this->general_m->get_data_anggotas($row->no_ktp);
			...
			<tr>
				...
				<td class="h_kiri"> : <strong>'.$row->lunas.'</strong></td>
			</tr>
		...
		$html .= '<strong> Simulasi Tagihan </strong><br /><br />';
		$html .= '<table width="100%">
		...
		$html .= '<br /><br /><strong> Data Pembayaran </strong>';
		if(!empty($angsuran)) {
			$html .='<br><br><table width="100%" cellspacing="0" cellpadding="2" border="1" border-collapse= "collapse">
			<tr class="header_kolom" >
				<th style=" width:5%;"> No. </th>
				<th style=" width:12%;"> Kode Bayar</th>
				<th style=" width:15%;"> Tanggal Bayar</th>
				<th style=" width:12%;"> Angsuran Ke </th>
			...
			foreach ($angsuran as $rows) {
				...
				$jml_bunga      += $rows->bunga;
				$jml_adm      	+= $rows->biaya_adm;
				...
				<td class="h_kanan"> '.number_format(nsi_round($rows->bunga)).'</td>
