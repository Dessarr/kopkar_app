...
public function __construct() {
	...
	function cetak() {
     ...
     $html = '
		...
		$denda = $jml_denda->total_denda; 
		//$bulatan = $jml_tagihan->jml_total - ($jml_pinjaman->jml_total + $jml_bunga->jml_total + $jml_biaya_adm->jml_total);
		$tagihan = ($jml_biaya_adm->jml_total + $jml_bunga->jml_total + $jml_denda->total_denda);
		...
		'<h3> Estimasi Data Pinjaman </h3>
			<table width="100%" cellspacing="0" cellpadding="3" border="1">
				...
				<tr>
					...
					<td> Pendapatan Biaya Denda</td>
					<td class="h_kanan">'.number_format(nsi_round($denda)) .'</td>
				</tr>
				<tr class="header_kolom">
					<td colspan="2" class="h_kanan">Jumlah</td>
					<td class="h_kanan">'.number_format($tagihan).'</td>
				</tr>
				<tr>
					<td colspan="2" class="h_kanan">Estimasi Pendapatan Pinjaman</td>
					<td class="h_kanan"><strong>'.number_format(nsi_round($tagihan)) .'</strong></td>
				</tr>			
			</table>
					';
