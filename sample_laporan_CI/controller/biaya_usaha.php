...
class Biaya_usaha extends OperatorController {
	...
	public function index() {
		...
		$this->data['kas_id'] = $this->biaya_usaha_m->get_data_kas();
		$this->data['akun_id'] = $this->biaya_usaha_m->get_data_akun();
		$this->data['isi'] = $this->load->view('biaya_usaha_list_v', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   		= $this->biaya_usaha_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$nama_kas = $this->biaya_usaha_m->get_jenis_kas($r->dari_kas_id);	
			$nama_akun = $this->biaya_usaha_m->get_jenis_akun($r->jns_trans);	
	...
	public function update($id=null) {
		...
		if($this->biaya_usaha_m->update($id)) {
	...
	public function delete() {
		...
		if($this->biaya_usaha_m->delete($id))
		{
	...
	function cetak_laporan() {
		$biaya_usaha = $this->biaya_usaha_m->lap_data_biaya_usaha();
		if($biaya_usaha == FALSE) {
			redirect('biaya_usaha');
			exit();
		}
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Laporan Data Biaya Usaha<br> </span>
		...
		$no =1;
		$jml_tot = 0;
		foreach ($biaya_usaha as $row) {
			$nama_kas = $this->biaya_usaha_m->get_jenis_kas($row->untuk_kas_id);	
			$nama_akun = $this->biaya_usaha_m->get_jenis_akun($row->jns_trans);	
