...
class P_pemasukan_kas extends OperatorController {
	...
	public function index() {
		...
		$this->data['kas_id'] = $this->p_pemasukan_m->get_data_kas();
		$this->data['akun_id'] = $this->p_pemasukan_m->get_data_akun();

		$this->data['isi'] = $this->load->view('p_pemasukan_list_v', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->p_pemasukan_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$nama_kas = $this->p_pemasukan_m->get_jenis_kas($r->untuk_kas_id);	
			$nama_akun = $this->p_pemasukan_m->get_jenis_akun($r->jns_trans);	
	...
	function cetak_laporan() {
		$p_pemasukan = $this->p_pemasukan_m->lap_data_pemasukan();
		if($p_pemasukan == FALSE) {
			redirect('p_pemasukan_kas');
			exit();
		}
		...
		<tr class="header_kolom">
			...
			<th class="h_tengah" style="width:10%;"> ID Cabang </th>
		</tr>';

		$no =1;
		$jml_tot = 0;
		foreach ($p_pemasukan as $row) {
			$nama_kas = $this->p_pemasukan_m->get_jenis_kas($row->untuk_kas_id);	
			$nama_akun = $this->p_pemasukan_m->get_jenis_akun($row->jns_trans);	
