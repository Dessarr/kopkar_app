...
class Piutang extends OperatorController {
	...
	public function index() {
		...
		$this->data['judul_sub'] 		= 'Piutang/Lain-lain Anggota';
		...
		$this->data['isi'] = $this->load->view('piutang_list_v', $this->data, TRUE);
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Piutang/Lain-lain <a href="'.site_url('piutang').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_shu')) {
		...
		$this->data['isi'] = $this->load->view('piutang_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}

	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('piutang_m','piutang', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->piutang->import_db($data_import)) {
	...
	function ajax_list() {
		...
		$data   = $this->piutang_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['id'] 				= $r->id;
			$rows[$i]['id_txt'] 			='TRD' . sprintf('%05d', $r->anggota_id) . '';
			...
			$rows[$i]['jumlah'] 			= number_format($r->jumlah_bayar);
			$rows[$i]['user'] 				= $r->user_name;
			$rows[$i]['nota'] 				= '<p></p><p>
			<a href="'.site_url('cetak_piutang').'/cetak/' . $r->id . '"  title="Cetak Bukti Piutang" target="_blank"> <i class="glyphicon glyphicon-print"></i> Nota </a></p>';
