...
class Skhusus2 extends OperatorController {
	public function __construct() {
		parent::__construct();	
		$this->load->helper('fungsi');
		$this->load->model('skhusus2_m');
	...
	public function index() {
		...
		$this->data['kas_id'] = $this->skhusus2_m->get_data_kas();
		$this->data['jenis_id'] = $this->general_m->get_id_simpanan();
		$this->data['isi'] = $this->load->view('skhusus2_list_v', $this->data, TRUE);
	...
	function list_anggota() {
		...
		//keys total & rows khusus2 bagi jEasyUI
	...
	function ajax_list() {
		...
		$cari_skhusus2 = isset($_POST['cari_skhusus2']) ? $_POST['cari_skhusus2'] : '';
		$tgl_dari = isset($_POST['tgl_dari']) ? $_POST['tgl_dari'] : '';
		$tgl_sampai = isset($_POST['tgl_sampai']) ? $_POST['tgl_sampai'] : '';
		$search = array('kode_transaksi' => $kode_transaksi, 
			'cari_skhusus2' => $cari_skhusus2,
			'tgl_dari' 		=> $tgl_dari, 
			'tgl_sampai' 	=> $tgl_sampai);
		$offset = ($offset-1)*$limit;
		$data   = $this->skhusus2_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		foreach ($data['data'] as $r) {
