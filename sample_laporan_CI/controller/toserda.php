...
class Toserda extends OperatorController {
	public function __construct() {
		parent::__construct();	
		$this->load->helper('fungsi');
		$this->load->model('toserda_m');
	...
	public function index() {
		...
		$this->data['judul_sub'] 		= 'TOSERDA Anggota';
		...
		//$this->data['kas_id'] = $this->shu_m->get_data_kas();
		//$this->data['jenis_id'] = $this->general_m->get_id_simpanan();
		$this->data['isi'] = $this->load->view('toserda_list_v', $this->data, TRUE);
	...
	function import() {
		...
		$this->data['judul_sub'] = 'TOSERDA <a href="'.site_url('toserda').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_toserda')) {
		...
		$this->data['isi'] = $this->load->view('toserda_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}

	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('toserda_m','toserda', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->toserda->import_db($data_import)) {
	...
	function ajax_list() {
		...
		$cari_toserda = isset($_POST['cari_toserda']) ? $_POST['cari_toserda'] : '';
		$tgl_dari = isset($_POST['tgl_dari']) ? $_POST['tgl_dari'] : '';
		$tgl_sampai = isset($_POST['tgl_sampai']) ? $_POST['tgl_sampai'] : '';
		$search = array('kode_transaksi' => $kode_transaksi, 
			'cari_toserda' => $cari_toserda,
			'tgl_dari' 		=> $tgl_dari, 
			'tgl_sampai' 	=> $tgl_sampai);
		$offset = ($offset-1)*$limit;
		$data   = $this->toserda_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$nama_simpanan = $this->general_m->get_jns($r->jns_trans);
			...
			$rows[$i]['jenis_id_txt'] 		= $nama_simpanan->jns_trans;
			$rows[$i]['jenis_id'] 			= $r->jns_trans;
			...
			$rows[$i]['nota'] 				= '<p></p><p>
			<a href="'.site_url('cetak_toserda').'/cetak/' . $r->id . '"  title="Cetak Bukti TOSERDA" target="_blank"> <i class="glyphicon glyphicon-print"></i> Nota </a></p>';
	...
	function cetak_laporan() {
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Laporan Data TOSERDA Anggota <br></span>
