...
class Angsuran extends OperatorController {
	...
	public function index($master_id = NULL) {
		...
		$row_pinjam = $this->general_m->get_data_pinjam_3($master_id);
		$this->data['row_pinjam'] = $row_pinjam; 
		$this->data['data_anggota'] = $this->general_m->get_data_anggotas($row_pinjam->no_ktp);
	...
	function ajax_list($id = NULL) {
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['bunga'] 			= number_format(nsi_round($r->bunga));
	...
	function get_ags_ke($master_id) {
		...
		$jml_pinjaman = $row_pinjam->jumlah; #jml pinjaman
		...
		$sisa_tagihan = number_format(nsi_round($total_bayar - $sudah_bayar)); #sisa tagihan 
		...
		//$sisa_tagihan_num = $row_pinjam->sisa_pokok;
		...
		$bunga = number_format(nsi_round($row_pinjam->pokok_bunga));

		if($ags_ke==1){
			$biaya_adm = number_format(nsi_round($row_pinjam->biaya_adm));
		}else{
			$biaya_adm = 0;
		}
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Angsuran <a href="'.site_url('angsuran').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_angsuran')) {
		...
		$this->data['isi'] = $this->load->view('angsuran_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}

	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('angsuran_m','angsuran', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->angsuran->import_db($data_import)) {
