...
class Penjualan extends OperatorController {
	...
	public function index() {
		...
		$this->data['kas_id'] = $this->penjualan_m->get_data_kas();
		$this->data['akun_id'] = $this->penjualan_m->get_data_akun();

		$this->data['isi'] = $this->load->view('penjualan_list_v', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   = $this->penjualan_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$nama_kas = $this->penjualan_m->get_jenis_kas($r->untuk_kas_id);	
			$nama_akun = $this->penjualan_m->get_jenis_akun($r->jns_trans);	
	...
	function cetak_laporan() {
		$penjualan = $this->penjualan_m->lap_data_penjualan();
		if($penjualan == FALSE) {
			redirect('penjualan');
			exit();
		}
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Laporan Data Penjualan<br> </span>
		...
		foreach ($penjualan as $row) {
			$nama_kas = $this->penjualan_m->get_jenis_kas($row->untuk_kas_id);	
			$nama_akun = $this->penjualan_m->get_jenis_akun($row->jns_trans);	
