...
class Swajib extends OperatorController {
	...
	public function index() {
		...
		$this->data['kas_id'] = $this->swajib_m->get_data_kas();
		$this->data['jenis_id'] = $this->general_m->get_id_simpanan();
		$this->data['isi'] = $this->load->view('swajib_list_v', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$cari_swajib = isset($_POST['cari_swajib']) ? $_POST['cari_swajib'] : '';
		$tgl_dari = isset($_POST['tgl_dari']) ? $_POST['tgl_dari'] : '';
		$tgl_sampai = isset($_POST['tgl_sampai']) ? $_POST['tgl_sampai'] : '';
		$search = array('kode_transaksi' => $kode_transaksi, 
			'cari_swajib' => $cari_swajib,
			'tgl_dari' 		=> $tgl_dari, 
			'tgl_sampai' 	=> $tgl_sampai);
		$offset = ($offset-1)*$limit;
		$data   = $this->swajib_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		foreach ($data['data'] as $r) {
