...
class Shu extends OperatorController {
	public function __construct() {
		parent::__construct();	
		$this->load->helper('fungsi');
		$this->load->model('shu_m');
	...
	public function index() {
		...
		$this->data['judul_sub'] 		= 'SHU Anggota';
		...
		$this->data['isi'] = $this->load->view('shu_list_v', $this->data, TRUE);
	...
	function import() {
		...
		$this->data['judul_sub'] = 'SHU <a href="'.site_url('shu').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		$this->data['isi'] = $this->load->view('shu_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}

	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('shu_m','shu', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->shu->import_db($data_import)) {
	...
	function ajax_list() {
		...
		$cari_shu = isset($_POST['cari_shu']) ? $_POST['cari_shu'] : '';
		...
		$data   = $this->shu_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['nota'] 				= '<p></p><p>
			<a href="'.site_url('cetak_shu').'/cetak/' . $r->id . '"  title="Cetak Bukti SHU" target="_blank"> <i class="glyphicon glyphicon-print"></i> Nota </a></p>';
	...
	function cetak_laporan() {
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Laporan Data SHU Anggota <br></span>
