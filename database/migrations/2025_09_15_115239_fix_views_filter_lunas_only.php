...
return new class extends Migration
{
    ...
    {
        // Fix v_hitung_pinjaman_detil - hanya tampilkan pinjaman yang belum lunas
        ...
        DB::statement("
            ...
            SELECT 
                ...
                -- Angsuran bulan ini (jika masih dalam masa pinjam)
                CASE 
                    ...
                    THEN FLOOR(a.jumlah / a.lama_angsuran) + 
                         CASE 
                             WHEN (YEAR(CURDATE()) - YEAR(a.tgl_pinjam)) * 12 + 
                                  (MONTH(CURDATE()) - MONTH(a.tgl_pinjam)) + 1 = a.lama_angsuran
                             THEN a.jumlah - (FLOOR(a.jumlah / a.lama_angsuran) * a.lama_angsuran)
                             ELSE 0
                         END
                    ELSE 0
                END as angsuran_bulan_ini,
                
                -- Tag bulan lalu (outstanding dari bulan sebelumnya)
                CASE 
                    WHEN a.lunas = 'Belum' AND 
                         (YEAR(CURDATE()) - YEAR(a.tgl_pinjam)) * 12 + 
                         (MONTH(CURDATE()) - MONTH(a.tgl_pinjam)) + 1 > a.lama_angsuran
            ...
            WHERE a.status = '1'  -- Hanya pinjaman yang disetujui
              AND a.lunas = 'Belum'  -- HANYA PINJAMAN YANG BELUM LUNAS
        ");
    }
