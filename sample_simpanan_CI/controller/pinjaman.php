...
class Pinjaman extends OperatorController {
	...
	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('pinjaman_m','pinjaman', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->pinjaman->import_db($data_import)) {
	...
	function get_anggota_by_id() {
		...
		if($r->file_pic == '') {
			...
			.'<br> ID : '.'AG' . sprintf('%04d', $r->id) . '<br /><input type="text" name="no_ktp" value="'.$r->no_ktp.'" />';
		} else {
	...
	function ajax_list() {
		...
		foreach ($data['data'] as $r) {
			...
			if($r->jenis_pinjaman == '1') {
				$jp = 'Biasa';
			} else if($r->jenis_pinjaman == '2'){
				$jp = 'Bank';
			} else {
				$jp = '-';
			}
			...
			$rows[$i]['anggota_id'] 		= $anggota->nama;
			$rows[$i]['jenis_pinjaman'] 	= $r->jenis_pinjaman;
			$rows[$i]['no_ktp']	 			= $anggota->no_ktp;
			$rows[$i]['anggota_id_txt'] 	= 'AG' . sprintf('%04d', $r->anggota_id).' <br />'.$anggota->nama;
			$rows[$i]['anggota_id_txt'] 	= 'AG' . sprintf('%04d', $r->anggota_id).' <br />'.$anggota->nama.' <br />'.$anggota->no_ktp.' <br />Pinjaman: '.$jp;
			...
			$rows[$i]['hitungan'] = '<table>
						<tr>
							<td width="100px" align="left">Pinjaman</td> 
						...
						<tr>
							...
							<td width="10px" align="center"> : </td>
							<td width="75px" align="right">'.number_format(nsi_round($r->bunga_rp)).'</td>
						</tr>
						<tr>
			...
			$rows[$i]['tagihan'] = '<table>
						...
						<tr>
							...
							<td width="10px" align="center"> : </td>
							<td width="75px" align="right">0</td>
						</tr>
