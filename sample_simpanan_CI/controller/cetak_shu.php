...
class Cetak_shu extends OperatorController {
	public function __construct() {
		parent::__construct();	
		$this->load->helper('fungsi');
		$this->load->model('cetak_shu_m');
	...
	function cetak($id) {
		$simpanan = $this->cetak_shu_m->lap_data_simpanan($id);
		...
		$html .= ''.$pdf->nsi_box($text =' <table width="100%">
			...
			<tr>
				<td width="20%"><strong>BUKTI PEMBAGIAN SHU</strong>
					<hr width="100%">
				</td>
		...
		foreach ($simpanan as $row) {
			$anggota= $this->cetak_shu_m->get_data_anggotas($row->no_ktp);
			$jns_simpan= $this->cetak_shu_m->get_jenis_simpan($row->jenis_id);
        ...
        //'.'AG'.sprintf('%04d', $row->anggota_id).'
			...
			<tr>
				<td> Terbilang </td> 
				<td>:</td>
				<td colspan="3">'.$this->terbilang->eja($row->jumlah_bayar).' RUPIAH </td>
			</tr>';
		}
		$html .= '</table> 
	...
	function cetak_all() {
		$simpanan = $this->cetak_shu_m->lap_data_simpanan_all();
