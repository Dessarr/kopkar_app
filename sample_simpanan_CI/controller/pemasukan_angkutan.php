...
class Pemasukan_angkutan extends OperatorController {
	...
	public function __construct() {
		...
		$this->load->model('pemasukan_angkutan_m');
	...
	public function index() {
		...
		$this->data['judul_sub'] 		= 'Pemasukan Angkutan';
		...
		$this->data['kas_id'] = $this->pemasukan_angkutan_m->get_data_kas();
		$this->data['akun_id'] = $this->pemasukan_angkutan_m->get_data_akun();
		//$this->data['nopol'] = $this->pemasukan_angkutan_m->get_data_nopol();

		$this->data['isi'] = $this->load->view('pemasukan_angkutan_list_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}

	function list_nopol() {
		$q = isset($_POST['q']) ? $_POST['q'] : '';
		$data   = $this->general_m->get_data_nopol_ajax($q);
		$i	= 0;
		$rows   = array(); 
		foreach ($data['data'] as $r) {
			if($r->file_pic == '') {
				...
				$rows[$i]['photo'] = '<img src="'.base_url().'uploads/mobil/' . $r->file_pic . '" alt="Foto" width="30" height="40" />';
			}
			$rows[$i]['id'] 	= $r->no_polisi;
			$rows[$i]['nopol'] 	= $r->no_polisi;
			$rows[$i]['nama'] 	= $r->nama;
			$rows[$i]['merek'] 	= $r->merek;
			$i++;
		}
	...
	function get_nopol_by_id() {
		$id = isset($_POST['nopol_id']) ? $_POST['nopol_id'] : '';
		$r   = $this->general_m->get_data_nopol($id);
		$out = '';
		$photo_w = 3 * 30;
		$photo_h = 4 * 30;
		if($r->file_pic == '') {
			$out ='<img src="'.base_url().'assets/theme_admin/img/photo.jpg" alt="default" width="'.$photo_w.'" height="'.$photo_h.'" />'
			.'<br /> ID : '.'' . sprintf('%04d', $r->id) . '<br /><input type="hidden" name="no_polisi" value="'.$r->no_polisi.'" />';
		} else {
			$out = '<img src="'.base_url().'uploads/mobil/' . $r->file_pic . '" alt="Foto" width="'.$photo_w.'" height="'.$photo_h.'" />'
			.'<br /> ID : '.'' . sprintf('%04d', $r->id) . '<br /><input type="hidden" name="no_polisi" value="'.$r->no_polisi.'" />';
		}
		echo $out;
		exit();
	...
	function ajax_list() {
		...
		$sort  = isset($_POST['sort']) ? $_POST['sort'] : 'tgl_catat';
		...
		$data   = $this->pemasukan_angkutan_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$nama_kas = $this->pemasukan_angkutan_m->get_jenis_kas($r->untuk_kas_id);	
			$nama_akun = $this->pemasukan_angkutan_m->get_jenis_akun($r->jns_trans);	

			$rows[$i]['id'] 				= $r->id;
			$rows[$i]['id_txt'] 			='PA' . sprintf('%05d', $r->id) . '';
			$rows[$i]['nopol_txt'] 			= $r->no_polisi;
	...
	public function create() {
		...
		if($this->pemasukan_angkutan_m->create()){
	...
	public function update($id=null) {
		...
		if($this->pemasukan_angkutan_m->update($id)) {
	...
	public function delete() {
		...
		if($this->pemasukan_angkutan_m->delete($id))
		{
	...
	function cetak_laporan() {
		$pemasukan = $this->pemasukan_angkutan_m->lap_data_pemasukan();
		if($pemasukan == FALSE) {
		...
		<tr class="header_kolom">
			...
			<th class="h_tengah" style="width:20%;"> Uraian  </th>
		...
		foreach ($pemasukan as $row) {
			$nama_kas = $this->pemasukan_angkutan_m->get_jenis_kas($row->untuk_kas_id);	
			$nama_akun = $this->pemasukan_angkutan_m->get_jenis_akun($row->jns_trans);	
