...
class Pengeluaran_kas extends OperatorController {
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Pengeluaran Kas <a href="'.site_url('pengeluaran_kas').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_pengeluaran_kas')) {
		...
		$this->data['isi'] = $this->load->view('pengeluaran_kas_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}

	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('pengeluaran_m','simpanan', TRUE);
			...
			redirect('pengeluaran_kas/import');
		} else {
			$this->session->set_flashdata('import', 'NO');
			redirect('pengeluaran_kas/import');
		}
