...
class Pengeluaran_angkutan extends OperatorController {

	public function __construct() {
		parent::__construct();	
		$this->load->helper('fungsi');
		$this->load->model('pengeluaran_angkutan_m');
	...
	public function index() {
		...
		$this->data['kas_id'] = $this->pengeluaran_angkutan_m->get_data_kas();
		$this->data['akun_id'] = $this->pengeluaran_angkutan_m->get_data_akun();

		$this->data['isi'] = $this->load->view('pengeluaran_angkutan_list_v', $this->data, TRUE);
	...
	function import() {
		...
		$this->data['judul_sub'] 		= 'Pengeluaran Angkutan <a href="'.site_url('pengeluaran_angkutan').'" class="btn btn-sm btn-success">Kembali</a>';
		...
		if($this->input->post('submit')) {
			...
			if ( ! $this->upload->do_upload('import_pengeluaran_angkutan')) {
		...
		$this->data['isi'] = $this->load->view('pengeluaran_angkutan_import_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}

	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('pengeluaran_angkutan_m','simpanan', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->pengeluaran->import_db($data_import)) {
			...
			redirect('pengeluaran_angkutan/import');
		} else {
			$this->session->set_flashdata('import', 'NO');
			redirect('pengeluaran_angkutan/import');
		}
	...
	function ajax_list() {
		...
		$data   = $this->pengeluaran_angkutan_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
	...
	public function create() {
		...
		if($this->pengeluaran_angkutan_m->create()){
	...
	public function update($id=null) {
		...
		if($this->pengeluaran_angkutan_m->update($id)) {
	...
	public function delete() {
		...
		if($this->pengeluaran_angkutan_m->delete($id))
		{
	...
	function cetak_laporan() {
		$pengeluaran = $this->pengeluaran_angkutan_m->lap_data_pengeluaran();
		if($pengeluaran == FALSE) {
			redirect('pengeluaran_angkutan');
			exit();
		}
