...
class Lap_pengeluaran_pinjaman extends OperatorController {
	public function __construct() {
		parent::__construct();
		$this->load->helper('fungsi');
		$this->load->model('lap_pengeluaran_pinjaman_m');
	...
	public function index() {
		...
		$this->data['judul_sub'] = 'Pengeluaran Pinjaman';
		...
		$config = array();
		$config["base_url"] 		= base_url() . "lap_pengeluaran_pinjaman/index/halaman";
		if (count($_GET) > 0) $config['suffix'] = '?' . http_build_query($_GET, '', "&");
		$config['first_url'] 		= $config['base_url'].'?'.http_build_query($_GET);
		$config["total_rows"] 		= $this->lap_pengeluaran_pinjaman_m->get_jml_data_kas();
		$config["per_page"] 		= 20;
		...
		$this->data["data_kas"] = $this->lap_pengeluaran_pinjaman_m->get_transaksi_pinjaman($config["per_page"], $offset);
		$this->data["saldo_awal"] = $this->lap_pengeluaran_pinjaman_m->get_saldo_awal($config["per_page"], $offset);
		$this->data["saldo_sblm"] = $this->lap_pengeluaran_pinjaman_m->get_saldo_sblm();
		$this->data["halaman"] = $this->pagination->create_links();
		$this->data["offset"] = $offset;

		//$this->data["pinjaman"] = $this->lap_pinjaman_m->get_transaksi_pinjaman(); 
		
		$this->data['isi'] = $this->load->view('lap_pengeluaran_pinjaman_list_v', $this->data, TRUE);
		$this->load->view('themes/layout_utama_v', $this->data);
	}

	function cetak_laporan() {
		$data_pinjam = $this->pengeluaran_pinjaman_m->lap_data_pinjaman();
	...
	function cetak() {
		$saldo_sblm = $this->lap_pengeluaran_pinjaman_m->lap_transaksi_pinjaman();
		if($saldo_sblm == FALSE) {
			echo 'DATA KOSONG';
			exit();
		}
		...
		$saldo = 0;
			...
			<tr class="header_kolom">
				...
				<th class="h_tengah" style="width:15%; vertical-align: middle "> Tanggal Pinjam </th>
				<th class="h_tengah" style="width:20%; vertical-align: middle "> Nama </th>
				<th class="h_tengah" style="width:10%; vertical-align: middle "> Pinjaman </th>
				<th class="h_tengah" style="width:20%; vertical-align: middle "> Jaminan</th>
				<th class="h_tengah" style="width:25%; vertical-align: middle "> Alamat </th>
				<th class="h_tengah" style="width:15%; vertical-align: middle "> No. Telp </th>
			</tr>';
			$no = 1;
			$d = 0;
			$k = 0;
			foreach ($saldo_sblm as $rows) {
				...
				$html.= '<tr>
					...
					<td> '.$rows->no_ktp.'<br />'.$rows->nama.'</td>
					<td class="h_kanan"> '.number_format(nsi_round($rows->jumlah)).' </td>
					<td class="h_kiri"> '.$rows->keterangan.' </td>
					<td class="h_kiri"> '.$rows->alamat.' </td>
					<td class="h_tengah"> '.$rows->notelp.' </td>
				</tr>';
			}
			$html.= '</table>';

		$nilai_d = $this->lap_pengeluaran_pinjaman_m->get_jml_d();
		$d = $nilai_d->jml_total;
		...
		$pdf->nsi_html($html);
		$pdf->Output('lap_pengeluaran_pinjaman'.date('Ymd_His') . '.pdf', 'I');
	}
}