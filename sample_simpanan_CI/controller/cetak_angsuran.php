...
class Cetak_angsuran extends OperatorController {
	...
	foreach ($angsuran as $row) {
		$pinjaman = $this->general_m->get_data_pinjams($row->id);
		$data_pinjam = $this->general_m->data_pinjamans($row->id);
		$get_total = $this->general_m->get_total($row->pinjam_id);
		...
		$jml_denda = $pinjaman->denda_rp;
		$hitung_dibayar = $this->general_m->get_jml_bayar($row->pinjam_id);
		$dibayar = $hitung_dibayar->total;
		$tagihan = $pinjaman->ags_per_bulan;

		$row_pinjam = $this->general_m->get_data_pinjamans ($row->pinjam_id);
		$total_bayar = $get_total->jumlah - $dibayar;

		//$sisa_bayar = $pinjaman->jumlah_bayar;
		...
		<tr>
			<td> ID Anggota </td>
			<td>:</td>
			<td>AG'.sprintf('%04d', $anggota->id).' / '.strtoupper($anggota->nama).'</td>
		...
		<tr>
			<td> No KTP </td>
			<td>:</td>
			<td class="h_kiri">'.$anggota->no_ktp.'</td>
		</tr>
		<tr>
			<td> Nomor Kontrak </td>
			<td >:</td>
	...
	<table width="100%">
		<tr>
			...
			<td width="5%">: Rp. </td>
			<td width="15%"  class="h_kanan">'.number_format($pinjaman->ags_per_bulan).'</td>

			<td width="17%"></td>
			<td width="16.5%">Denda </td>
			<td width="5%">: Rp. </td>
		...
		<tr>
			...
			<td width="5%">: Rp. </td>
			<td class="h_kanan">'.number_format($pinjaman->bunga).'</td>

			<td width="17%"> </td>
			<td width="16.5%">Sisa Pinjaman</td>
			<td width="5%">: Rp. </td>
			<td width="20%" class="h_kanan">'.number_format(nsi_round($total_bayar)).'</td>
		</tr>
		<tr>
			<td> Biaya Admin </td>
			<td>: Rp. </td>
			...
			<td width="17%"></td>
			<td width="16.5%">Total Bayar </td>
			<td width="5%">: Rp. </td>
			<td width="20%" class="h_kanan">'.number_format(nsi_round($row->jumlah_bayar + $pinjaman->bunga + $pinjaman->biaya_adm + $jml_denda)).'</td>
		</tr>
		<tr>
