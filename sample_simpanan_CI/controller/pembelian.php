...
class Pembelian extends OperatorController {
	...
	public function index() {
		...
		$this->data['kas_id'] = $this->pembelian_m->get_data_kas();
		$this->data['akun_id'] = $this->pembelian_m->get_data_akun();

		$this->data['isi'] = $this->load->view('pembelian_list_v', $this->data, TRUE);
	...
	function ajax_list() {
		...
		$data   		= $this->pembelian_m->get_data_transaksi_ajax($offset,$limit,$search,$sort,$order);
		$i	= 0;
		$rows   = array(); 
		...
		foreach ($data['data'] as $r) {
			...
			$nama_kas = $this->pembelian_m->get_jenis_kas($r->dari_kas_id);	
			$nama_akun = $this->pembelian_m->get_jenis_akun($r->jns_trans);	
	...
	function cetak_laporan() {
		$pembelian = $this->pembelian_m->lap_data_pembelian();
		if($pembelian == FALSE) {
			redirect('pembelian');
			exit();
		}
		...
		'.$pdf->nsi_box($text = '<span class="txt_judul">Laporan Data Pembelian<br> </span>
		...
		foreach ($pembelian as $row) {
			$nama_kas = $this->pembelian_m->get_jenis_kas($row->untuk_kas_id);	
			$nama_akun = $this->pembelian_m->get_jenis_akun($row->jns_trans);	
