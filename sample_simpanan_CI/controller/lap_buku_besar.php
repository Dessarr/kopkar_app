...
class Lap_buku_besar extends OperatorController {
	...
	function cetak_excel() {
        ...
        $spreadsheet->getActiveSheet()
			->setCellValue('A1', "LAPORAN BUKU BESAR ".$txt_periode);
		$spreadsheet->getActiveSheet()
			->mergeCells("A1:G1");
		$spreadsheet->getActiveSheet()
			->getStyle('A1')
		...
		foreach ($nama_kas as $key) {
			...
			$spreadsheet->setActiveSheetIndex(0)
	            ...
	            ->setCellValue('C2', 'Jenis Transaksi')
			...
			foreach ($transD as $rows) {
				...
				$spreadsheet->setActiveSheetIndex(0)
                	...
                	->setCellValue('B' . $kolom, $txt_tanggalD)
                	->setCellValue('C' . $kolom, @$nm_akun->jns_trans)
                	->setCellValue('D' . $kolom, $rows->nama)
                	->setCellValue('E' . $kolom, nsi_round($rows->debet))
                	->setCellValue('F' . $kolom, nsi_round($rows->kredit))
                	->setCellValue('G' . $kolom, nsi_round($saldo));
                $kolom++;
			}
		}
