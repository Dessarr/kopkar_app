...
class Member extends CI_Controller {
		...
		if ($this->session->userdata('login') == TRUE && $this->session->userdata('level') == 'member') {
			...
			if($this->member->validasi()) {
				if($this->member->cek_user()) {
					...
					$this->data['pesan'] = 'Username atau Password salah. INI SUDAH SELESAI BELUM SIH >>> ??';
				}
			} else {
         }
         $this->data['jenis'] = 'member';
         $this->load->view('themes/member_login_form_v', $this->data);
		}
	}
	...
	function simulasi() {
		...
		if($pinjaman == 'bunga_biasa') {
			$persen_bunga = $conf_bunga['bunga_biasa'];
		} else {
			$persen_bunga = $conf_bunga['bunga_barang'];
		}
		//$persen_bunga = $jumlah;
		...
		for ($i=1; $i <= $lama_ags; $i++) { 
			...
			// --- ONLY FIRST ANGSURAN WILL CHARGE ADMIN FEE ----
			if($i==1){
				$odat['biaya_adm'] = number_format($biaya_admin);
			}else{
				$odat['biaya_adm'] = number_format(0);
				$biaya_admin = 0;
			}
	...
	function edit_penarikan() {
		$this->load->model('pinjaman_m');
		$res = $this->pinjaman_m->penarikan_edit();
		echo $res;
	}
	...
	public function view() {
		...
		$this->data['data_penarikan'] 		= $this->member->get_last_penarikan();
	...
	public function penarikan_simpanan() {
		...
		$this->data['jenis_ags'] 		= $this->pinjaman_m->get_data_angsuran();
		$this->load->view('themes/member_pengajuan_penarikan_v', $this->data);		
	}
	...
	public function ajax_penarikan() {
		$this->load->model('member_m');
	...
	public function pengajuan_batal_penarikan($id) {
		$this->_cek_login();
		$this->load->model('member_m');
		if($this->member_m->pengajuan_batal_penarikan($id)) {
			$this->session->set_flashdata('ajuan_batal', 'Y');
		} else {
			$this->session->set_flashdata('ajuan_batal', 'N');
		}
		redirect('member/penarikan_simpanan');
	...
	public function pengajuan_baru() {
		...
		$this->data['pinjaman']=$this->pinjaman_m->ambil_pinjaman();
		$this->load->helper('fungsi');
		$this->load->view('themes/member_pengajuan_baru_v', $this->data);		
	}

	public function pengajuan_penarikan_baru() {
		$this->_cek_login();
		//number_format
		$this->data['js_files'][] = base_url() . 'assets/extra/fungsi/number_format.js';
		$this->load->model('pinjaman_m');
		...
		$this->data['tersimpan'] = '';
		if ($this->input->post('submit')) {
			if($this->member->validasi_penarikan()) {
				$pengajuan_penarikan = $this->member->pengajuan_penarikan();
				if($pengajuan_penarikan) {
					$this->session->set_flashdata('penarikan_baru', 'Y');
					redirect('member/penarikan_simpanan');
				} else {
					$this->data['tersimpan'] = 'N';
				}
		...
		$this->data['simpanan']=$this->pinjaman_m->ambil_simpanan();
		$this->load->helper('fungsi');
		$this->load->view('themes/member_pengajuan_penarikan_baru_v', $this->data);		
	}

	function ambil_jumlah(){
		$this->load->model('pinjaman_m');
        $id_simpanan = $this->input->post('id_simpanan');
        $this->data = $this->pinjaman_m->ambil_jumlah($id_simpanan);
        echo json_encode($this->data);
    }

    function ambil_persen(){
		$this->load->model('pinjaman_m');
        $id_pinjaman = $this->input->post('id_pinjaman');
        $this->data = $this->pinjaman_m->ambil_persen($id_pinjaman);
        echo json_encode($this->data);
    }
	...
	public function lap_bayar_simpanan() {
		$this->_cek_login();
		$this->load->helper('fungsi');
		$this->load->view('themes/member_bayar_simpanan_v', $this->data);
	}
	...
	public function ajax_lap_bayar_simpanan() {
		$this->load->model('member_m');
		$out = $this->member_m->get_bayar_simpanan();
		header('Content-Type: application/json');
		echo json_encode($out);
		exit();		
	}
