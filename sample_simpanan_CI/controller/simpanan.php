...
class Simpanan extends OperatorController {
	...
	function import() {
		...
		$this->data['judul_sub'] = 'Simpanan <a href="'.site_url('simpanan').'" class="btn btn-sm btn-success">Kembali</a>';
	...
	function import_db() {
		if($this->input->post('submit')) {
			$this->load->model('simpanan_m','simpanan', TRUE);
			$data_import = $this->input->post('val_arr');
			if($this->simpanan->import_db($data_import)) {
	...
	function ajax_list() {
		...
		foreach ($data['data'] as $r) {
			...
			$rows[$i]['no_ktp']	 			= $r->no_ktp;
			$rows[$i]['anggota_id_txt'] 	= 'AG' . sprintf('%04d', $anggota->id);
