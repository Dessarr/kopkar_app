...
class Anggota extends OperatorController {
	...
	public function index() {
		...
		$crud->columns('file_pic','no_ktp','nama','jk','alamat','id_tagihan','aktif');
		$crud->fields('nama','identitas','jk', 'tmp_lahir','tgl_lahir','status','departement','pekerjaan','agama','alamat','kota','notelp','tgl_daftar','jabatan_id','pass_word','aktif','file_pic','no_ktp','bank','nama_pemilik_rekening','no_rekening','id_tagihan','simpanan_pokok','simpanan_wajib','simpanan_sukarela','simpanan_khusus_2');
		$crud->display_as('id_anggota','ID Anggota');
		//$crud->display_as('id_anggota','ID Anggota');
		$crud->display_as('identitas','Username');
		$crud->display_as('nama','Nama Lengkap');
		$crud->display_as('id_tagihan','ID Karyawan');
		...
		$crud->display_as('no_ktp','ID Koperasi');

		$crud->display_as('bank','BANK');
		$crud->display_as('nama_pemilik_rekening','Nama Pemilik Rekening');
		$crud->display_as('no_rekening','No Rekening');
		...
		$crud->required_fields('nama','identitas','jk','aktif','no_ktp');
		...
		$this->db->select('bank');
		$this->db->from('jns_bank');
		$query = $this->db->get();
		if($query->num_rows()>0){
			$result = $query->result();
			foreach ($result as $val) {
				$bank[$val->bank] = $val->bank;
			}
		} else {
			$kerja = array('' => '-');
		}
		$crud->field_type('bank','dropdown',$bank);
