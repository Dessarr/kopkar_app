...
class Angsuran_lunas extends OperatorController {
	...
	public function index($master_id = NULL) {
		...
		$this->data['bunga'] = $this->general_m->get_data_pinjam($master_id);
	...
	function ajax_list($id=NULL) {
		...
		foreach ($data['data'] as $r) {
			...
			$anggota = $this->general_m->get_data_anggotas($id_pinjam->no_ktp); 
			...
			$rows[$i]['bunga'] 			= number_format($r->bunga);
			$rows[$i]['ket_bayar'] 		= $r->ket_bayar;
			$rows[$i]['kas_id'] 		= $r->kas_id;
			$rows[$i]['user'] 			= $r->user_name;
			$rows[$i]['id_cabang'] 		= $this->session->userdata('id_cabang');
			$rows[$i]['nota'] 			= '<p></p><p>
