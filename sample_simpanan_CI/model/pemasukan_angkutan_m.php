...
class Pemasukan_angkutan_m extends CI_Model {
...
//panggil data jenis kas
	...
	function get_data_transaksi_ajax($offset, $limit, $q='', $sort, $order) {
		$sql = "SELECT * FROM tbl_trans_kas WHERE akun='Pemasukan' and jns_trans=46 ";
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
				$q['kode_transaksi'] = str_replace('TKD', '', $q['kode_transaksi']);
				$q['kode_transaksi'] = $q['kode_transaksi'] * 1;
	...
	public function create() {
		...
		$data = array(
			//'id_mobil'		=>	$this->input->post('nopol_id'),
			...
			'no_polisi'		=>	$this->input->post('nopol_id'),
