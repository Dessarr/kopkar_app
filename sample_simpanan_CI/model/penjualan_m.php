...
//panggil data jenis kas
	...
	function get_data_akun() {
		$id_akun = array('112','113','114','115','116');
		$this->db->select('*');
	...
	function lap_data_penjualan() {
		...
		$sql = '';
		$sql = " SELECT * FROM tbl_trans_kas WHERE akun='Pemasukan' and jns_trans in ('112','113','114','115','116') ";
	...
	function get_data_transaksi_ajax($offset, $limit, $q='', $sort, $order) {
		...
		$sql = "SELECT * FROM tbl_trans_kas WHERE jns_trans in ('112','113','114','115','116') and akun='Pemasukan' ";
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
				$q['kode_transaksi'] = str_replace('TKD', '', $q['kode_transaksi']);
				$q['kode_transaksi'] = $q['kode_transaksi'] * 1;
