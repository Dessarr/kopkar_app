...
class Tagihan_m extends CI_Model {
	...
	function get_data_transaksi_ajax($offset, $limit, $q='', $sort, $order) {
		//$id_cab = $this->session->userdata('id_cabang');
		$sql = "SELECT * FROM v_tagihan WHERE jenis_id in (8,31,32,40,41,51,52) ";
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
				...
				if($q['cari_tagihan'] != '') {
					$sql .=" AND jenis_id = '".$q['cari_tagihan']."%' ";
				}
	...
	public function create() {
		...
		return $this->db->insert('tbl_trans_tagihan', $data);
	}
	...
	public function update($id) {
		...
		$this->db->where('id', $id);
		return $this->db->update('tbl_trans_tagihan',array(
			'tgl_transaksi'		=>	$this->input->post('tgl_transaksi'),
			'no_ktp'			=>	$this->input->post('no_ktp'),
			'jenis_id'			=>	$this->input->post('jenis_id'),
			'jumlah'			=>	str_replace(',', '', $this->input->post('jumlah')),
	...
	public function delete($id) {
		return $this->db->delete('tbl_trans_tagihan', array('id' => $id)); 
	}
}