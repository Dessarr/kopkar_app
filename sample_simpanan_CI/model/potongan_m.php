...
class Potongan_m extends CI_Model {
		...
		if(is_array($data)) {
			...
			foreach ($data as $rows) {
				...
				foreach ($rows as $key => $val) {
					...
					if($key == 'D') { $pair['dk'] = 'K'; }
					if($key == 'E') { $pair['kas_id'] = '1'; }
					if($key == 'F') { $pair['user_name'] = 'admin'; }
	...
	function lap_data_simpanan() {
		...
		$sql = '';
		$sql = " SELECT * FROM v_shu WHERE dk='K' ";
	...
	function get_data_transaksi_ajax($offset, $limit, $q='', $sort, $order) {
		//$id_cab = $this->session->userdata('id_cabang');
		$sql = "SELECT * FROM tbl_trans_sp WHERE jenis_id='125' ";
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
				$q['kode_transaksi'] = str_replace('TRD', '', $q['kode_transaksi']);
				$q['kode_transaksi'] = str_replace('AG', '', $q['kode_transaksi']);
