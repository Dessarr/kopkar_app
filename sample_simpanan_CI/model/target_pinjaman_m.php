...
class Target_pinjaman_m extends CI_Model {
	...
	function get_pengajuan() {
		...
		$sql_tampil = "SELECT 
			a.id AS id, b.id_cabang AS id_cabang, a.no_ajuan AS no_ajuan, a.ajuan_id AS ajuan_id, a.anggota_id AS anggota_id, a.tgl_input AS tgl_input, a.jenis AS jenis, a.nominal AS nominal, a.lama_ags AS lama_ags, a.keterangan AS keterangan, a.status AS status, a.alasan AS alasan, a.tgl_update AS tgl_update, a.tgl_cair AS tgl_cair,
			...
			LEFT JOIN tbl_anggota AS b ON b.id = a.anggota_id
		 	WHERE b.id_cabang='$id_cab' and 1=1 ".$where." ".$order_by." ".$sql_limit."";
		$query = $this->db->query($sql_tampil);
		$data_list = $query->result();

		$sql_total = "SELECT id FROM tbl_pengajuan AS a WHERE id_cabang='$id_cab' and 1=1 ".$where." ";
		...
		if(is_array($data)) {
			...
			foreach ($data as $rows) {
				...
				foreach ($rows as $key => $val) {
					...
					if($key == 'D') { $pair['anggota_id'] = $val; }
					if($key == 'E') { $pair['barang_id'] = $val; }
					if($key == 'F') { $pair['lama_angsuran'] = $val; }
					if($key == 'G') { $pair['jumlah'] = $val; }
					if($key == 'H') { $pair['bunga'] = $val; }
					if($key == 'I') { $pair['biaya_adm'] = $val; }
					if($key == 'J') { $pair['lunas'] = $val; }
					if($key == 'K') { $pair['dk'] = $val; }
					if($key == 'L') { $pair['kas_id'] = $val; }
					if($key == 'M') { $pair['jns_trans'] = $val; }
					if($key == 'N') { $pair['update_data'] = $val; }
					if($key == 'O') { $pair['user_name'] = $val; }
					if($key == 'P') { $pair['keterangan'] = $val; }
					if($key == 'Q') { $pair['contoh'] = $val; }
					if($key == 'R') { $pair['id_cabang'] = $id_cab; }
				}
	...
	function get_pengajuan_cetak() {
		...
		$sql_tampil = "SELECT 
			...
			FROM tbl_pengajuan AS a
			LEFT JOIN tbl_anggota AS b ON b.id = a.anggota_id
		 	WHERE b.id_cabang='$id_cab' and 1=1 ".$where." ".$order_by." ";
		$query = $this->db->query($sql_tampil);
		$data_list = $query->result();
	...
	function get_data_pengajuan($id) {
		$id_cab = $this->session->userdata('id_cabang');
		$sql_tampil = "SELECT 
			a.id_cabang AS id_cabang, a.id AS id, a.anggota_id AS anggota_id, a.tgl_input AS tgl_input, a.jenis AS jenis, a.nominal AS nominal, a.lama_ags AS lama_ags, a.keterangan AS keterangan, a.status AS status, a.alasan AS alasan, a.tgl_update AS tgl_update, a.tgl_cair AS tgl_cair,
			b.identitas AS identitas, b.nama AS nama, b.departement AS departement
			FROM tbl_pengajuan AS a
			LEFT JOIN tbl_anggota AS b ON b.id = a.anggota_id
		 	WHERE a.id_cabang='$id_cab' and a.id = ".$id."";
		$query = $this->db->query($sql_tampil);
		if($query->num_rows() > 0) {
			$out = $query->row();
			return $out;
		...
		if($row) {
			...
			for ($i=1; $i <= $row->lama_angsuran; $i++) { 
				...
				*/
				...
				$odat['bunga_pinjaman'] = $row->pokok_bunga;
	...
	function get_data_transaksi_ajax($id_cab, $offset, $limit, $q='', $sort, $order) {
		...
		$where = " WHERE dk = 'K' and id_cabang='$id_cab' ";
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
					...
					if($q['cari_nama'] != '') {
						$where .=" AND v_hitung_pinjaman.nama LIKE '%".$q['cari_nama']."%' ";
	...
	function lap_data_pinjaman() {
		...
		$tgl_sampai = isset($_REQUEST['tgl_sampai']) ? $_REQUEST['tgl_sampai'] : '';
		$sql = '';
		$sql = " SELECT * FROM v_hitung_pinjaman WHERE id_cabang='$id_cab' and dk = 'K' ";
