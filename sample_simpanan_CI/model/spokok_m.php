...
class Spokok_m extends CI_Model {
	...
	function get_data_transaksi_ajax($offset, $limit, $q='', $sort, $order) {
		$sql = "SELECT * FROM v_spokok where jns_simpan = 'Simpanan pokok' ";
		if(is_array($q)) {
			if($q['kode_transaksi'] != '') {
				...
				if($q['cari_spokok'] != '') {
					$sql .=" AND jenis_id = '".$q['cari_spokok']."%' ";
				}
	...
	public function view() {
		...
		$view = $this->db->query("SELECT * FROM v_spokok where YEAR(tgl_transaksi) = '$thn' and MONTH(tgl_transaksi) = '$bln'");
        return $view;
	}
	...
	public function load() {
		...
		$this->db->query("DELETE FROM `tbl_trans_tagihan_pokok_temp` WHERE YEAR(tgl_transaksi) = '$thn' and MONTH(tgl_transaksi) = '$bln'");
		
		$load = $this->db->query("INSERT INTO tbl_trans_tagihan_pokok_temp (tgl_transaksi, no_ktp, anggota_id, jenis_id, jumlah, keterangan, akun, dk) 
			SELECT '$thn-$bln-01', a.no_ktp, a.id, '40', a.simpanan_pokok, '', 'Tagihan', 'D'
			FROM tbl_anggota a 
			WHERE a.aktif='Y'
			and Year(a.tgl_daftar) = '$thn'
                        and Month(a.tgl_daftar) = '$bln'");
        return $load;
	}
	...
	public function ins() {
		...
		$this->db->query("DELETE FROM tbl_trans_tagihan WHERE YEAR(tgl_transaksi) = '$thn' and MONTH(tgl_transaksi) = '$bln' and jenis_id = '40'");
		$this->db->query("INSERT INTO tbl_trans_tagihan (tgl_transaksi, no_ktp, anggota_id, jenis_id, jumlah, keterangan, akun, dk) 
			...
			FROM tbl_trans_tagihan_pokok_temp 
			WHERE YEAR(tgl_transaksi) = '$thn' and MONTH(tgl_transaksi) = '$bln'");

		$load = $this->db->query("DELETE FROM `tbl_trans_tagihan_pokok_temp` WHERE YEAR(tgl_transaksi) = '$thn' and MONTH(tgl_transaksi) = '$bln'");
        return $load;
	}
	...
	public function create() {
		...
		return $this->db->insert('tbl_trans_tagihan_pokok_temp', $data);
	}
	...
	public function update($id) {
		...
		$this->db->where('id', $id);
		return $this->db->update('tbl_trans_tagihan_pokok_temp',array(
			'tgl_transaksi'		=>	$this->input->post('tgl_transaksi'),
			'no_ktp'			=>	$this->input->post('no_ktp'),
			'jenis_id'			=>	40,
			'jumlah'			=>	str_replace(',', '', $this->input->post('jumlah')),
	...
	public function delete($id) {
		return $this->db->delete('tbl_trans_tagihan_pokok_temp', array('id' => $id)); 
	}
}